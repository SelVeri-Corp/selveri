...
\title{SelVeri: Self-Verifying Programming Language \\ High Level Language Syntax and Semantics Document \\ v0.3}
...
\[
\begin{array}{rcl}
\langle \mathit{Ident} \rangle &::=& [A\texttt{-}Za\texttt{-}z\_][A\texttt{-}Za\texttt{-}z0\texttt{-}9\_]^* \\
...
\mathtt{Float}
\;\mid\;
\mathtt{List}\,[\langle \mathit{Type} \rangle,\langle \mathit{AExp} \rangle]
\end{array}
\]
...
Here the syntax is also constructed in a way that establishes operator precedence rules.
\[
\begin{array}{rcl}
...
&::=&
\langle \mathit{Ident} \rangle[\langle \mathit{AExp} \rangle]\;\texttt{:=}\;\langle \mathit{AExp} \rangle
\end{array}
\]
...
&::=&
\langle \mathit{Ident} \rangle\;\texttt{:}\;\langle \mathit{Type} \rangle \\[6pt]
...
\mathtt{pass}\texttt{;}
\;\mid\;
\langle \mathit{ReturnStmt} \rangle\texttt{;} \\[4pt]
...
&::=&
\mathtt{return}\;\langle \mathit{Expr} \rangle
\end{array}
\]
...
\footnotetext{Although not explicitly stated in its grammar, every \texttt{SelVeri} function body must end with a \texttt{return} statement. However, if the user does not put a \texttt{return} statement at the end of the function body; instead of throwing a compilation error, the compiler automatically puts one, returning the default value of the functions return-type.}
...
\begin{itemize}
    ...
    \item $ \textbf{List}: (\Pi{t} : \textbf{Type} \, . \,  (\Pi{n} : \textbf{$\mathbb{N}^+$} \, . \,  \textbf{Type}))$
\end{itemize}

\subsubsection{Immediates}

Let \textbf{Imm} be the set of syntactic immediate values supported by \texttt{SelVeri}, e.g. $5$ or  $3.14$. For each type, we need an evaluation function that translates syntactic immediates into the corresponding semantic values.
...
\]
\[
\mathcal{L}_{\textbf{List[$t, n$]}}: \textbf{Imm} \cap \textbf{List[$t, n$]} \rightarrow A^n 
\]
where $A$ is the codomain of $\mathcal{L}_t$. The reader should notice that:

\begin{itemize}
    \item $\mathcal{L}_\textbf{Int} = \mathcal{I}$
    \item $\mathcal{L}_\textbf{Float} = \mathcal{F}$
\end{itemize}
...
Let \textbf{Scope} be the set of all scopes and let \textbf{Var} be the infinite set of all syntactic variables. Each scope is a representation  of 'known' variables in a given block of statements including the variables declared until program execution reaches the corresponding block. Formally,

\[
\forall s \in \textbf{Scope} \, . \, s: \textbf{Var} \,\cup\, \textbf{Imm}  \rightarrow \textbf{Type}
\]
Given any scope, $s$, immediates are mapped to the corresponding types; e.g. $s(5) = \textbf{Int}$, , as expected.\\
...
\]
where \textbf{Void} is the empty type.\\
...
s(x) = \textbf{List[$t,n$]}
\implies
\forall i \in \{0, \,...\, , n-1\} \,.\, s(x\texttt{[$i$]}) = t
\]
...
Moreover, each scope comes along with a (possibly empty) \texttt{parent\_scope}, which is the scope of the program block that 'called' the current block. In the \texttt{SelVeri} implementation, a variable is searched in the \texttt{parent\_scope}, if it cannot be found in the current scope.
...
Each program starts with an empty state, $\tilde{\sigma}$ in which each declared variable has the default value of this type, e.g. 0 for \textbf{Int}, 0.0 for \textbf{Float} and  [0, 0, 0] for \textbf{List[Int, 3]}. If the specific type is unknown in a given context, the default value of the corresponding type is denoted as \textbf{0}.\\
...
Let \textbf{Func} be the infinite set of all possible definable \texttt{SelVeri} functions. To store the information of all function definitions in a given program, we introduce the concept of the \textit{function environment}, denoted as $\Gamma$:

\[
\Gamma : \textbf{Func} \rightarrow (\textbf{Var} \times \textbf{Type})^n \times \textbf{Code}
...
\[
\texttt{function f(x : Int, y : Float, z : List[Float,2]) -> Int ::($S_\textbf{f}$)} 
\]
where $S_\textbf{f}$ is some function statement, then 
\[
\Gamma(\texttt{f}) = ((\texttt{x},\, \texttt{Int}), (\texttt{y},\, \texttt{Float}),(\texttt{z},\, \texttt{List[Float,2]})),\; S_\textbf{f})
\]
...
\] Assuming that, with the inputs above, execution of the body of \texttt{f} does not result in an error state, our operational semantics will ensure that variable \texttt{w} holds the result of the computation of \texttt{f}.


\newpage
\subsection{Semantics of arithmetic expressions}

Let \textbf{AExp} be the set of all arithmetic expressions. We define the arithmetic valuation function $\mathcal{A}$, representing the semantics of the arithmetic expressions in \texttt{SelVeri}.

\[
\mathcal{A} : \textbf{AExp} \times \textbf{State} \times \textbf{Scope} \rightarrow \textbf{Val} 
\]

\begin{table}[h!]
\centering
\renewcommand{\arraystretch}{1.8} % increases vertical space
\large % increases overall font size in the table

% \makebox forces the oversized box to center perfectly on the page
\makebox[\textwidth][c]{% 
\fbox{%
\begin{tabular}{l}
% Internal array to align the first four "ifs"
$\begin{array}{ll}
\mathcal{A}(x, \sigma, s)= \mathcal{I}(x) & \qquad \text{if } x \in \textbf{Imm} \land s(x) = \textbf{Int}\\

\mathcal{A}(x, \sigma, s)= \mathcal{F}(x) & \qquad \text{if } x \in \textbf{Imm} \land s(x) = \textbf{Float}\\

\mathcal{A}(x, \sigma, s)= \mathcal{L}_{\textbf{List[$t, n$]}}(x) & \qquad \text{if } x \in \textbf{Imm} \land s(x) = \textbf{List[} t,n \textbf{]} \\

\mathcal{A}(x, \sigma, s)= \sigma(x, s) & \qquad \text{if } x \in \textbf{Var} 

\end{array}$ \\

\(\mathcal{A}(\texttt{len($x$)}, \sigma, s)= \left\{
  \begin{array}{ll}
    0& \text{if } s(x) \in \{\textbf{Int}, \textbf{Float}\} \\
    n& \text{if } \exists t \in \textbf{Type} \cdot \exists n \in \mathbb{N}^+ \cdot  s(x) = \textbf{List[$t,n$]} \\
    \bot & \text{otherwise}
  \end{array}
\right.\) \\

\(\mathcal{A}(a_1 + a_2, \sigma, s)=  
\left\{
  \begin{array}{ll}
    \mathcal{A}(a_1, \sigma, s)+ \mathcal{A}(a_2, \sigma, s) & \text{if } \mathcal{A}(a_1, \sigma, s)\neq \bot \land \mathcal{A}(a_2, \sigma, s)\neq \bot \\
    \bot & \text{if } \mathcal{A}(a_1, \sigma, s)= \bot \lor \mathcal{A}(a_2, \sigma, s)= \bot
  \end{array}
\right.\) \\

\(\mathcal{A}(a_1 \ast a_2, \sigma, s)= \left\{
  \begin{array}{ll}
    \mathcal{A}(a_1, \sigma, s)\cdot \mathcal{A}(a_2, \sigma, s)& \text{if } \mathcal{A}(a_1, \sigma, s)\neq \bot \land \mathcal{A}(a_2, \sigma, s)\neq \bot \\
    \bot & \text{if } \mathcal{A}(a_1, \sigma, s)= \bot \lor \mathcal{A}(a_2, \sigma, s)= \bot
  \end{array}
\right.\) \\

\(\mathcal{A}(a_1 - a_2, \sigma, s)= \left\{
  \begin{array}{ll}
    \mathcal{A}(a_1, \sigma, s)- \mathcal{A}(a_2, \sigma, s)& \text{if } \mathcal{A}(a_1, \sigma, s)\neq \bot \land \mathcal{A}(a_2, \sigma, s)\neq \bot \\
    \bot & \text{if } \mathcal{A}(a_1, \sigma, s)= \bot \lor \mathcal{A}(a_2, \sigma, s)= \bot
  \end{array}
...
\(\mathcal{A}(a_1 / a_2, \sigma, s)= \left\{
  \begin{array}{ll}
    \mathcal{A} (a_1, \sigma, s) /  \mathcal{A}(a_2, \sigma, s) \footnotemark & \text{if } \mathcal{A}(a_2, \sigma, s)\neq 0 \land (\mathcal{A}(a_1, \sigma, s)\neq \bot \land \mathcal{A}(a_2, \sigma, s)\neq \bot) \\
    \bot & \text{if } \mathcal{A}(a_2, \sigma, s)= 0 \lor (\mathcal{A}(a_1, \sigma, s)= \bot \lor \mathcal{A}(a_2, \sigma, s)= \bot)  \\
  \end{array}
\right.\) \\

\end{tabular}%
}%
} % <-- Closing bracket for \makebox
...
\begin{table}[h!]
\centering
\renewcommand{\arraystretch}{2.0} % increase vertical spacing
...
\begin{table}[!ht]
\centering
\renewcommand{\arraystretch}{2.5} % Reduced vertical space so it fits under the heading
\large % Larger font size 
\resizebox{\textwidth}{!}{% Requires \usepackage{graphicx} in preamble
\fbox{%
\begin{tabular}{rl}

\([ \text{decl}_{\text{os}} ]\) &
\(\langle x \;\texttt{:}\; t, \sigma, s \rangle \rightarrow \langle \sigma[x \mapsto \textbf{0}] ,\, s[x \mapsto t] \rangle \quad \text{if } t \in \textbf{Type} \land s(x) = \textbf{Void}\) \\
...
\([ \text{if}^{1}_{\text{os}} ]\) &
\(\langle \texttt{if } b \texttt{ then } S_1 \texttt{ else } S_2 \texttt{ fi} \footnotemark, \sigma, s  \rangle \rightarrow \langle S_1, \sigma, s \rangle
\quad \text{if } \mathcal{B}(b, \sigma, s) = 1\) \\

\([ \text{if}^{0}_{\text{os}} ]\) &
\(\langle \texttt{if } b \texttt{ then } S_1 \texttt{ else } S_2 \texttt{ fi}, \sigma, s  \rangle \rightarrow \langle S_2, \sigma, s \rangle
...
\(\langle \texttt{while } b \texttt{ do } S \texttt{ od}, \sigma, s \rangle \rightarrow \langle S;\texttt{while } b \texttt{ do } S \texttt{ od}, \sigma, s \rangle
\quad \text{if } \mathcal{B}(b, \sigma, s) = 1\) \\
...
\quad \text{if } \mathcal{B}(b, \sigma, s) = \bot\) \\[2pt]

\([ \text{call}_{\text{os}} ]\) &
\renewcommand{\arraystretch}{1.2} % Local vertical space for this specific cell
\(\begin{array}[t]{@{}l@{}}
\langle \texttt{call}\; f(x_1, \dots, x_n), \sigma, s \rangle \rightarrow \langle x_1 : t_1 \texttt{;} \dots \texttt{;} x_n : t_n \texttt{;} S_f,\, \tilde{\sigma}', \tilde{s}' \rangle \\
\quad \text{if } \footnotemark t_i = s(x_i) \text{ for } i = 1, 2, \dots, n \\
\quad \text{where } \Gamma(f) = (((x'_1, t_1), \dots, (x'_n, t_n)), S_f)
\end{array}\) \\[2pt]
...
\end{table}
\footnotetext{For each if-rule, an \texttt{if}-statement without the \texttt{else}-part, namely a statement of the form "\texttt{if} $b$ \texttt{then} $S$ \texttt{fi}" can be treated as "\texttt{if} $b$ \texttt{then} $S$ \texttt{else pass fi}"}
\footnotetext{Otherwise, the function $f$ is not called an program state is transitioned into $\hat{\sigma}$.}
