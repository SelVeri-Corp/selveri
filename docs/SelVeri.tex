...
$
    \begin{alignedat}{2}
    \mathcal{A}(x, \sigma, s) &= \mathcal{I}(x)
    \qquad &&\text{if } x \in \textbf{Imm} \land s(x) = \textbf{Int} \\
    
    \mathcal{A}(x, \sigma, s) &= \mathcal{F}(x)
    \qquad &&\text{if } x \in \textbf{Imm} \land s(x) = \textbf{Float} \\
    
    \mathcal{A}(x, \sigma, s) &= \text{size}(\textbf{List}[ t,d, \vec{n}]) : \mathcal{L}_{\textbf{List}[t, d,\vec{n}]}(x)
    \qquad &&\text{if } x \in \textbf{Imm} \land s(x) = \textbf{List[} t,d,\vec{n}  \textbf{]} \\
    
    \mathcal{A}(x, \sigma, s) &= \sigma(x, s)
    \qquad &&\text{if } x \in \textbf{Var} \\
    
    \mathcal{A}(lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]}, \sigma, s) &= \mathcal{A}(lst\texttt{[$\mathcal{A}(i_1, \sigma, s)$]}\dots\texttt{[$\mathcal{A}(i_m, \sigma, s)$]}, \sigma, s)
    \qquad &&\text{if $lst \in \textbf{Var}$ and $lst$} \\

    \mathcal{A}(\texttt{len($x$)}, \sigma, s) &= 
    \left\{
      \begin{aligned}
        & 0 \\
        & \mathcal{A}(\texttt{\_$x$\_len\_1}, \sigma, s) \\
        & \bot
      \end{aligned}
    \right.
    \qquad
      &&\begin{aligned}  
        &\text{if } x \in \textbf{Var} \land s(x) \in \textbf{BasicType} \\
        &\text{if } x \in \textbf{Var} \land s(x) \notin \{\textbf{Void}, \textbf{Int}, \textbf{Float}\} \\
        &\text{otherwise}
      \end{aligned} \\
    ...
    \left\{
    \begin{aligned}
        & \mathcal{A}(a_1, \sigma, s) + \mathcal{A}(a_2, \sigma, s) \\
        & \bot
    \end{aligned}
    \right.
    \qquad
    &&\begin{aligned}
        &\text{if } \mathcal{A}(a_1, \sigma, s)\neq \bot \land \mathcal{A}(a_2, \sigma, s)\neq \bot \\
        &\text{if } \mathcal{A}(a_1, \sigma, s)= \bot \lor \mathcal{A}(a_2, \sigma, s)= \bot
    \end{aligned} \\

    \mathcal{A}(a_1 \ast a_2, \sigma, s) &= 
    \left\{
    \begin{aligned}
        & \mathcal{A}(a_1, \sigma, s)\cdot \mathcal{A}(a_2, \sigma, s) \\
        & \bot
    \end{aligned}
    \right.
    \qquad
    ...
    \left\{
    \begin{aligned}
        & \mathcal{A}(a_1, \sigma, s) / \mathcal{A}(a_2, \sigma, s) \footnotemark \\
        & \bot
    \end{aligned}
    \right.
    \qquad
    &&\begin{aligned}
        &\text{if } \mathcal{A}(a_2, \sigma, s)\neq 0 \land (\mathcal{A}(a_1, \sigma, s)\neq \bot \land \mathcal{A}(a_2, \sigma, s)\neq \bot) \\
        &\text{if } \mathcal{A}(a_2, \sigma, s)= 0 \lor (\mathcal{A}(a_1, \sigma, s)= \bot \lor \mathcal{A}(a_2, \sigma, s)= \bot)
    \end{aligned} \\
\end{alignedat}
$
\end{tabular}%
