...
\title{SelVeri: Self-Verifying Programming Language \\ High Level Language Syntax and Semantics Document \\ v0.8}
...
&::=&
\langle \mathit{Ident} \rangle[\langle \mathit{AExp} \rangle]\;\texttt{:=}\;\langle \mathit{AExp} \rangle \\[4pt]

\langle \mathit{IOStmt} \rangle
&::=&
\mathtt{read}\;\langle \mathit{Ident} \rangle 
...
For convenience, we define the projection function:
\[
\pi_d: (n_1,\cdots,n_d) \rightarrow (n_2,\cdots,n_d)
\]
...
\]
Given any scope, $s$, immediates are mapped to the corresponding types; e.g. $s(5) = \textbf{Int}$, , as expected. Additionally, given any non-atomic arithmetic expression $a$, we have

\[
s(a) = \begin{cases}
\textbf{Int} \quad \text{if $s(a') = \textbf{Int}$ for all subterms $a'$ of $a$} \\
\textbf{Float} \quad \text{if $s(a') = \textbf{Int}$ for some subterm $a'$ of $a$} \\ 
\end{cases}
\] Note that a composite arithmetic expression cannot have a non-basic type in \textit{SelVeri}. \\
...
\[
\Gamma : \textbf{Func} \rightarrow (\textbf{Var} \times \textbf{Type})^n \times \textbf{Type} \times \textbf{Code}
...
\[
\texttt{function f(x : Int, y : Float, z : List[Float,1,2]) -> Int ::($S_\textbf{f}$)} 
\]
...
\texttt{SelVeri} supports multi-dimensional lists and to be able to formalize them correctly we define the following helper function:
...
\mathcal{A}(x, \sigma, s)= size(\textbf{List[} t,d, \vec{n} \textbf{]}) : \mathcal{L}_{\textbf{List[$t, d,\vec{n} $]}}(x) \qquad \text{if } x \in \textbf{Imm} \land s(x) = \textbf{List[} t,d,\vec{n}  \textbf{]} \\

\mathcal{A}(x, \sigma, s)= \sigma(x, s) \qquad \text{if } x \in \textbf{Var} \\

\mathcal{A}(lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]}, \sigma, s)= \mathcal{A}(lst\texttt{[$\mathcal{A}(i_1, \sigma, s)$]}\dots\texttt{[$\mathcal{A}(i_m, \sigma, s)$]}, \sigma, s) & \\ \qquad \text{if $lst \in \textbf{Var}$ and $lst$} 
...
\(\mathcal{A}(\texttt{len($x$)}, \sigma, s)= \left\{
  \begin{array}{ll}
    ...
    \mathcal{A}(\texttt{\_$x$\_len\_1}, \sigma, s)& \text{if } x \in \textbf{Var} \land s(x) \notin \{\textbf{Void}, \textbf{Int}, \textbf{Float}\} \\
    \bot & \text{otherwise}
  \end{array}
\right.\) \\
...
\renewcommand{\arraystretch}{1.2}
\(\begin{array}[t]{@{}l@{}}
\langle \texttt{read}\; x, \sigma, s \rangle \rightarrow \langle \sigma[x \mapsto i],\; s\rangle \\
\quad \text{if $x \in \textbf{Var} \land i \in ran\mathcal{L}_{s(x)}$}\\
\quad \text{where $i$ is the external user input}
\end{array}\) \\[6pt]
