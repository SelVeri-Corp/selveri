...
\title{SelVerIR: Self-Verifying Programming Language \\ Intermediate Representation Syntax and Semantics Document \\ v0.8}
...
&& \mid\; \texttt{READ} \; \langle ident \rangle
\\[2pt]

&& \mid\; \texttt{WRITE} \; \langle ident \rangle
\\[2pt]

&& \mid\; \texttt{WRITELN} \; \langle ident \rangle
\\[2pt]
...
Let $P$ be any \texttt{SelVerIR} program throughout the section \textbf{2.2}. Note that any configuration which is not explicitly stated in the tables above leads to the erroneous state, $\hat{\sigma}$ and increments $pc$ by exactly 1.
...
\langle \texttt{READ}\; x : c ,\, e,\, \sigma,\, s,\, pc \rangle & 
{\renewcommand{\arraystretch}{1.0}
\begin{array}[t]{@{}l@{}}
\langle c,\, e,\, \sigma[x \mapsto i],\, s,\, pc+1 \rangle \\
\text{if $i$ is the input and $x \in \textbf{Var} \land i \in ran\mathcal{L}_{s(x)}$}
\end{array}} \\

\langle \texttt{WRITE}\; x : c ,\, e,\, \sigma,\, s,\, pc \rangle & 
{\renewcommand{\arraystretch}{1.0}
\begin{array}[t]{@{}l@{}}
\langle c,\, e,\, \sigma,\, s,\, pc+1 \rangle \\
\text{where the abstract machine emits the value $\mathcal{A}(x, \sigma, s)$}
\end{array}} \\

\langle \texttt{WRITELN}\; x : c ,\, e,\, \sigma,\, s,\, pc \rangle & 
{\renewcommand{\arraystretch}{1.0}
\begin{array}[t]{@{}l@{}}
\langle c,\, e,\, \sigma,\, s,\, pc+1 \rangle \\
\text{where the abstract machine emits the value $\mathcal{A}(x, \sigma, s)$ and a newline}
\end{array}} \\
\hline\hline
\end{tabular*}
...
Observe that the valuation of \texttt{size\_IR}($t$) is equal $size(t)$. With the help of $\mathit{LF}$ and \texttt{size\_IR}, we will be able to correctly define the compilation of list-accesses and list-declarations in \textbf{sections 3.2} and \textbf{3.4}.
...
\\ \quad = \texttt{ENV} : [\texttt{L}]\texttt{DECL}\; t_1\;x_1 : \texttt{STORE}\; x_1 :  \dots : [\texttt{L}]\texttt{DECL}\; t_n\;x_n : \texttt{STORE}\; x_n : \mathcal{C}(S_f)
\\[1.5ex]
...
\mathcal{C}(\texttt{read}\; x) = \texttt{READ}\;x
\\[1.5ex]

\mathcal{C}(\texttt{write}\; x) = \texttt{WRITE}\;x
\\[1.5ex]

\mathcal{C}(\texttt{writeline}\; x) = \texttt{WRITELN}\;x
\\[1.5ex]
\end{array}$%
}%
} % <-- closes \resizebox
...
\begin{itemize}
  ...
  \]
    ...
    This case is trivially true as $\texttt{LEN}\;x$ exaclty pushes the valuation of \texttt{len($x$)} to the stack.

    \item \textbf{case 7: } $a = a_1 + a_2$ for some $a_1, a_2 \in \textbf{AExp}$ \\
  ...
  \]
    ...
    \item \textbf{case 8: } $a = a_1 \ast a_2$ for some $a_1, a_2 \in \textbf{AExp}$
  ...
  \]
    ...
    \newpage
    \item \textbf{case 9: } $a = a_1 - a_2$ for some $a_1, a_2 \in \textbf{AExp}$
  ...
  \]
    ...
    \item \textbf{case 10.1: } $a = a_1 / a_2$ for some $a_1, a_2 \in \textbf{AExp}$ and $(a_1 / a_2)$ has no subterm of type \textbf{Float}

    Assume for an induction on the correctness of the compilation of $a_1$ and $a_2$.

        \begin{itemize}
            \item \textbf{subcase 10.1.1: } $\mathcal{A}(a_2, \sigma, s) \neq 0$
          ...
          \]
            ...
            \item \textbf{subcase 10.1.2: } $\mathcal{A}(a_2, \sigma, s) = 0$
    ...
    \item \textbf{case 10.2: } $a = a_1 / a_2$ for some $a_1, a_2 \in \textbf{AExp}$ and $(a_1 / a_2)$ has at least one subterm of type \textbf{Float}

    Assume for an induction on the correctness of the compilation of $a_1$ and $a_2$.

        \begin{itemize}
            \item \textbf{subcase 10.2.1: } $\mathcal{A}(a_2, \sigma, s) \neq 0.0$
          ...
          \]
            ...
            \item \textbf{subcase 10.2.2: } $\mathcal{A}(a_2, \sigma, s) = 0.0$
...
\begin{itemize}
  ...
  \]
    ...
    \item \textbf{(base) case 3:} $P$ is an IO-statement, i.e. $P \in \{\texttt{read}\;x ,\, \texttt{write}\;x ,\, \texttt{writeline}\; x \}$. Then, the compilation of IO-statement is trivially true as they compile into one IR-command whose semantics apply the exact same change to the program state and scope.
