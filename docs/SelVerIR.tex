...
\title{SelVerIR: Self-Verifying Programming Language \\ Intermediate Representation Syntax and Semantics Document \\ v0.5}
...
&::=& \texttt{DECL} \; \langle type \rangle \; , \langle ident \rangle \;\mid\; \texttt{LDECL} \; \langle ldecl\_type \rangle \; , \langle ident \rangle
\\[2pt]

&& \mid\; \texttt{PUSH} \; \langle imm \rangle \;\mid\; \texttt{PUSH} \; \langle ident \rangle
\\[2pt]

&& \mid\; \texttt{STORE} \; \langle ident \rangle
\\[2pt]

&& \mid\; \texttt{ACCESS} \; \langle ident \rangle
\\[2pt]

&& \mid\; \texttt{INSERT} \; \langle ident \rangle
\\[2pt]
...
\langle \mathit{type} \rangle &::=& \texttt{INT} \;\mid\; \texttt{FLOAT} \;\mid\; \texttt{LIST[INT},\langle \mathit{int} \rangle \texttt{]} \;\mid\; \texttt{LIST[FLOAT},\langle \mathit{int} \rangle \texttt{]}   \\[6pt]

\langle \mathit{ldecl\_type} \rangle &::=& \texttt{LIST[INT]} \;\mid\; \texttt{LIST[FLOAT]}  \\[6pt]
...
At any moment of the execution of a \texttt{SelVerIR} program, its stack, denoted as $e$, is a an ordered finite sequence consisting elements are from \textbf{Val}, which will be used in the flow of the program in the \textit{LIFO} order. As expected, $e$ supports \texttt{PUSH} and (indirectly) \texttt{POP} operations. For convenience, the empty stack is denoted separately, as $\epsilon$.

\subsubsection{Program counter}

Intuitively, one can think of any \texttt{SelverIR} program, $P$ as an ordered list of commands. Program counter, $pc$, represents the index of the command that is to be executed next. Control-flow statements such as \texttt{GOTO} and \texttt{JZ} can alter the program counter in a non-sequential manner.
...
Let $P$ be any \texttt{SelVerIR} program throughout the section \textbf{2.2}. Note that any configuration which is not explicitly stated in the tables above lead to the erroneous state, $\hat{\sigma}$.
...
\begin{cases}
\langle c,\, e,\, \sigma[x \mapsto \textbf{0}],\, s[x \mapsto t],\, pc+1 \rangle \\ \qquad \text{if } x \in \textbf{Var} \;\land\; t \in \textbf{Type} \;\land\; s(x) = \textbf{Void}\\
\end{cases} \\

\langle \texttt{LDECL}\;t\,x : c ,\, n : e,\, \sigma,\, s,\, pc \rangle & 
\begin{cases}
\langle c,\, e,\, \sigma[x \mapsto \textbf{0}],\, s[x \mapsto \texttt{LIST[INT, $n$]}],\, pc+1 \rangle \\ \qquad \text{if } x \in \textbf{Var} \;\land\; t \in \texttt{LIST[INT]}]\;\land\; s(x) = \textbf{Void}\\

\langle c,\, e,\, \sigma[x \mapsto \textbf{0}],\, s[x \mapsto \texttt{LIST[FLOAT]}],\, pc+1 \rangle \\ \qquad \text{if } x \in \textbf{Var} \;\land\; t \in \texttt{LIST[FLOAT]}]\;\land\; s(x) = \textbf{Void}\\
...
\langle c,\, \sigma(x\texttt{[0]},s) : \dots : \sigma(x\texttt{[n-1]},s): e,\, \sigma,\, s,\, pc+1 \rangle & \\ \qquad \text{if } s(x) = \texttt{LIST[$t, n$]} \land x \in \textbf{Var} \\[2pt]
\langle c,\, e,\, \hat{\sigma},\, s,\, pc+1 \rangle \qquad \text{otherwise} \\[2pt]
\langle c,\, \mathcal{V}(x) : e,\, \sigma,\, s,\, pc+1 \rangle \qquad \text{if } x \in \textbf{Imm}
\end{cases} \\


\langle \texttt{STORE}\; x : c,\, v_1 : \dots : v_n : e,\, \sigma,\, s,\, pc \rangle & \begin{cases}
\langle c,\, e,\, \sigma[x \mapsto v_1],\, s,\, pc+1 \rangle & \\ \qquad \text{if } s(x) \in \{\texttt{INT}, \texttt{FLOAT}\} \land n=1 \land v_1 \in \mathbb{R} \land x \in \textbf{Var} \\[2pt]
\langle c,\, e,\, \sigma[x \mapsto [v_1, \dots, v_n]],\, s,\, pc+1 \rangle & \\ \qquad \text{if } s(x) = \texttt{LIST[$t, m$]}\land m \leq n \land v_1, \dots v_n \in \mathbb{R} \land x \in \textbf{Var} \\[2pt]
\langle c,\, e,\, \hat{\sigma},\, s,\, pc+1 \rangle \qquad \text{otherwise} \\
\end{cases}\\


\langle \texttt{ACCESS}\;x : c,\, i : e,\, \sigma,\, s,\, pc \rangle & \begin{cases}
\langle c,\, \sigma(x\texttt{[$i$]}, s) : e,\, \sigma,\, s,\, pc+1 \rangle & \\ \qquad \text{if } s(x) = \texttt{LIST[$t,n$]} \land 0 \leq i \leq n \land i,v \in \mathbb{N} \land x \in \textbf{Var} \\[2pt]
\langle c,\, e,\, \hat{\sigma},\, s,\, pc+1 \rangle \qquad \text{otherwise} \\
\end{cases}\\


\langle \texttt{INSERT}\;x : c,\, i : v : e,\, \sigma,\, s,\, pc \rangle & \begin{cases}
\langle c,\, e,\, \sigma[x\texttt{[$i$]} \mapsto v],\, s,\, pc+1 \rangle & \\ \qquad \text{if } s(x) = \texttt{LIST[$t,n$]} \land 0 \leq i \leq n \land i,v \in \mathbb{N} \land x \in \textbf{Var} \\[2pt]
\langle c,\, e,\, \hat{\sigma},\, s,\, pc+1 \rangle \qquad \text{otherwise} \\
\end{cases}\\
...
\langle c,\, n : e,\, \sigma,\, s,\, pc+1 \rangle & \text{if } s(x) = \texttt{LIST[t, $n$]} \\
\langle c,\, \bot : e,\, \hat{\sigma},\, s,\, pc+1 \rangle & \text{otherwise}
\end{cases} \\
...
As one can notice from the table above, we do not have type-specific commands for \texttt{ADD}, \texttt{SUB} and \texttt{MUL} as these operations are closed under $\mathbb{Z}$ \footnote{Although syntactically different in \texttt{SelVeri} high level language; semantically, we do not interpret any integer differently than its real number counterpart.}. However, since this is not the case for division, we have separate instructions for integer and float division, namely \texttt{iDIV} and \texttt{fDIV}.
A similar approach will be followed for the boolean expressions in the next section.
...
\langle \texttt{ENV} : c ,\, e,\, \sigma,\, s,\, pc \rangle & \langle c,\, e,\, \tilde{\sigma}',\, \tilde{s}',\, pc+1 \rangle \quad \text{where $\sigma$ and $s$ is of the caller}  \\

\langle \texttt{RET} : c ,\, v : e,\, \sigma',\, s',\, pc ,\, ra : ras \rangle & \langle P[ra:],\, e,\, \sigma[\texttt{retvar} \mapsto v],\, s[\texttt{retvar} \mapsto s'(v)],\, ra ,\, ras \rangle \quad \text{where $\sigma$ and $s$ is of the caller} \\
...
\subsection{Compilation of types and variables}
...
\[
\mathcal{CT}: \textbf{Type} \rightarrow \textbf{TypeIR}
\] where \textbf{TypeIR} is the set of all types supported by \texttt{SelVerIR}, e.g. \texttt{INT}, \texttt{FLOAT} and one-dimensional (finite) lists of the former two. Note that $\mathcal{CT}$ omits size if the input list type does not have an explicit size.
...
\fbox{%
$\begin{array}{ll}
\mathcal{CT}(\textbf{Int}) = \texttt{INT}
\\[1ex]

\mathcal{CT}(\textbf{Float}) = \texttt{FLOAT}
\\[1ex]

\mathcal{CT}(\textbf{List[$t, n$]}) = 
\begin{cases}
\texttt{LIST[INT, $n$]} & \text{if } \mathcal{CT}(t) = \texttt{INT} \\
\texttt{LIST[FLOAT, $n$]} & \text{if } \mathcal{CT}(t) = \texttt{FLOAT} \\
\texttt{LIST[INT, $n \times m$]} & \text{if } \mathcal{CT}(t) = \texttt{LIST[INT, $m$]} \\
\texttt{LIST[FLOAT, $n \times m$]} & \text{if } \mathcal{CT}(t) = \texttt{LIST[FLOAT, $m$]}
\end{cases}
\\[1ex]
...
To ensure consistency, with type compilations and \textit{list flattening}, we need to formalize it:

\[
\mathit{I}: \textbf{AExp} \rightarrow \textbf{AExp}
\]

\[
\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m) = \sum_{j=1}^{m} i_j \left( \prod_{k=j+1}^{m} n_k \right)
\]

$\mathcal{CT}$ and $\mathit{I}$ are obviously well-defined, however we need to prove that $\mathit{I}$ is injective on indices as well to ensure that the compilation is correct.

\begin{proof}
Even though not explicitly stated in its formal definition, all $i_j$ represent indices and $n_k$ represent length of the dimension $k$. As $\mathit{I}$ will solely be used to flatten a valid indexing of an $m$-dimensional list, we assume standard 0-based indexing, meaning each index $i_k$ is bounded by $0 \le i_k < n_k$.


Let $W_j = \prod_{k=j+1}^{m} n_k$ represent the weight of the $j$-th dimension. Note that $W_m = 1$, and $W_{j-1} = n_j W_j$.

We can rewrite the index calculation as:
$$\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m)= i_1 W_1 + i_2 W_2 + \dots + i_m W_m$$

To prove injectivity, we must show that if $\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m) = I(i'_1, \dots, i'_m ;n_1,\dots,n_m)$, then $i_j = i'_j$ for all $1 \le j \le m$.

We can prove this by examining the maximum possible value of any suffix sum of the index calculation. The maximum value for the remaining dimensions from $r+1$ to $m$ occurs when every index is at its maximum possible value ($n_j - 1$):
$$\text{Max Sum} = \sum_{j=r+1}^{m} (n_j - 1) W_j$$

Because $W_{j-1} = n_j W_j$, this sum elegantly telescopes:
$$\text{Max Sum} = ((n_{r+1} - 1) W_{r+1}) + \dots + ((n_m - 1) W_m)$$
$$\text{Max Sum} = W_r - W_m = W_r - 1$$

This demonstrates a crucial property: the sum of all lower-dimensional components ($i_{r+1} W_{r+1} + \dots + i_m W_m$) is strictly less than the weight of the current dimension $W_r$.

Because the remainder is strictly smaller than the step size of the current dimension, it is impossible for a change in a lower dimension to overflow and alter a higher dimension. Therefore, two different sets of indices must produce different flat indices, proving that $\mathit{I}$ is injective for $m$-dimensional lists. \\
\end{proof}

As the well-definedness of $\mathcal{CT}$ and injectivity of $\mathit{I}$ is established, we will omit them to for convenience, except when we explicitly define other compilation functions.
...
\fbox{%
$\begin{array}{ll}
\mathcal{CA}(x) = \texttt{PUSH}\;x \qquad \text{if } x \in \textbf{Imm} \cap (\textbf{Int} \cup \textbf{Float})\\[1ex]

\mathcal{CA}(x) = \texttt{PUSH}\; x \qquad \text{if } x \in \textbf{Var} \text{ and $x$ is of a basic type} \\[1ex]

\mathcal{CA}(lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]}) = \mathcal{CA}(\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m)) : \texttt{ACCESS}\;x \\ \qquad \text{if $lst$ is an $m$-dimensional list where each dimension $k$ has length $n_k$} \\[1ex]
...
\mathcal{C}(x \;\texttt{:}\; t) = \texttt{DECL}\; \mathcal{CT}(t)\;x
\\[1.5ex]

\mathcal{C}(x \;\texttt{:=}\; a) = \mathcal{CA}(a) : \texttt{STORE}\;x \qquad \text{if } x \in \textbf{Var} \text{ and $x$ is of a basic type}
\\[1.5ex]

\mathcal{C}(lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]} \;\texttt{:=}\; a) = \mathcal{CA}(a) : \mathcal{CA}(\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m)) :  \texttt{INSERT}\;lst \\ \qquad \text{if } lst \in \textbf{Var} \text{ and $lst$ is an $m$-dimensional list where each dimension $k$ has length $n_k$}
\\[1.5ex]
...
\mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{NOOP}
\\[1.5ex]

\mathcal{C}(\texttt{if } b \texttt{ then } S_1 \texttt{ else } S_2 \texttt{ fi}) = 
\mathcal{CB}(b) : \texttt{JZ}\; pc_{S_2} : \mathcal{C}(S_1) : \texttt{GOTO}\; pc_{\texttt{NOOP}} : \mathcal{C}(S_2) : \texttt{NOOP}
\\[1.5ex]

\mathcal{C}(\texttt{while } b \texttt{ do } S \texttt{ od}) = 
\mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP}
\\[1.5ex]

\mathcal{C}(\texttt{function}\; f(x_1 : t_1 \dots, x_n : t_n) \,\texttt{->}\, t_r \,\texttt{::}\, S_f) 
\\ \quad = \texttt{ENV} : [\texttt{L}]\texttt{DECL}\; \mathcal{CT}(t_1)\;x_1 : \texttt{STORE}\; (x_1) :  \dots : [\texttt{L}]\texttt{DECL}\; \mathcal{CT}(t_n)\;(x_n) : \texttt{STORE}\; x_n : \mathcal{C}(S_f)
\\[1.5ex]

\mathcal{C}(\texttt{call}\; f(a_1 \dots, a_n)) = 
\mathcal{CA}(a_n) : [\texttt{LEN}\;a_n :] \dots : \mathcal{CA}(a_1) : [\texttt{LEN}\;a_1 :]  \texttt{CALL}\; f
\\[1.5ex]
...
Regarding the in-brackets parts of function definition and function call compilations, to give the flexibility of defining functions that accepts lists of any size, we allow declaration of  list parameters without an explicitly stated size. To cover this up, a new instruction called \texttt{LDECL} in introduced, which expects the size of the declared list to be on top of the stack, instead of statically determined by the compiler (or user). The choice between \texttt{DECL} and \texttt{LDECL} depends solely how the function in context is defined, whether the list has an (explicitly stated) static size or a dynamic size. However, since in both cases (with or without in-brackets parts) the resulting program state and scope tuple is trivially equivalent, we omit this distinction.
...
\begin{itemize}
  ...
  \]
    ...
    \item \textbf{case 2: } $a = x$ for some $x \in \textbf{Var}$ and $x$ is of a basic type

    \[
      \begin{aligned}
        \langle  \mathcal{CA}(a) : c,\, e,\, \sigma,\, s,\, pc \rangle 
        &= \langle \mathcal{CA}(x) : c,\, e,\, \sigma,\, s,\, pc \rangle \\
        &= \langle \texttt{PUSH}\;x : c,\, e,\, \sigma,\, s,\, pc \rangle 
        && \text{(definition of $\mathcal{CA}$)} \\
  ...
  \item \textbf{case 3: } $a = lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]}$ for some $m$-dimensional $lst$ where each dimension $k$ has length $n_k$

  Suppose $\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m)) = j$ for some $j \in \mathbb{N}$ and assume for an induction on the correctness of the compilation of $j$. 

    \[
      \begin{aligned}
        \langle  \mathcal{CA}(a) : c,\, e,\, \sigma,\, s,\, pc \rangle 
        &= \langle \mathcal{CA}(lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]}) : c,\, e,\, \sigma,\, s,\, pc \rangle \\
        &= \langle \mathcal{CA}(\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m)) : \texttt{ACCESS}\;x : c,\, e,\, \sigma,\, s,\, pc \rangle 
        && \text{(definition of $\mathcal{CA}$)} \\
        &= \langle \mathcal{CA}(j) : \texttt{ACCESS}\;x : c,\, e,\, \sigma,\, s,\, pc \rangle 
        && \text{(definition of $\mathit{I}$)} \\
        &\triangleright^\ast \langle \texttt{ACCESS}\;x : c,\, \mathcal{A}(j, \sigma, s):  e,\, \sigma,\, s,\, pc + l_j \rangle 
        && \text{(induction hypothesis)} \\
        &\triangleright \langle c,\, \sigma(lst\texttt{[$\mathcal{A}(j, \sigma, s)$]}, s):  e,\, \sigma,\, s,\, pc + l_j + 1 \rangle
        && \text{(operational semantics)} \\
        &= \langle c,\, \sigma(lst\texttt{[$\mathcal{A}(\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m), \sigma, s)$]}, s):  e,\, \sigma,\, s,\, pc + l_j + 1 \rangle
        && \text{(definition of $\mathit{I}$)} \\
        &= \langle c,\, \mathcal{A}(lst\texttt{[$\mathcal{A}(i_1, \sigma, s)$]}\dots\texttt{[$\mathcal{A}(i_m, \sigma, s)$]}, \sigma, s) : e,\, \sigma,\, s,\, pc+1 \rangle 
        && \text{(list flattening)} \\
        &= \langle c,\, \mathcal{A}(lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]}, \sigma, s) : e,\, \sigma,\, s,\, pc+1 \rangle
        && \text{(definition of $\mathcal{A}$)} \\
        &= \langle c,\, \mathcal{A}(a, \sigma, s) : e,\, \sigma,\, s,\, pc+1 \rangle \\
      \end{aligned}
  \]
   ...
   \newpage 
    \item \textbf{case 4: } $a = a_1 + a_2$ for some $a_1, a_2 \in \textbf{AExp}$ \\
  ...
  \]
    ...
    \item \textbf{case 5: } $a = a_1 \ast a_2$ for some $a_1, a_2 \in \textbf{AExp}$
  ...
  \]
    ...
    \item \textbf{case 6: } $a = a_1 - a_2$ for some $a_1, a_2 \in \textbf{AExp}$
  ...
  \]
    ...
    \item \textbf{case 7.1: } $a = a_1 / a_2$ for some $a_1, a_2 \in \textbf{AExp}$ and $(a_1 / a_2)$ has no subterm of type \textbf{Float}

    Assume for an induction on the correctness of the compilation of $a_1$ and $a_2$.

        \begin{itemize}
            \item \textbf{subcase 7.1.1: } $\mathcal{A}(a_2, \sigma, s) \neq 0$
          ...
          \]
            ...
            \item \textbf{subcase 7.1.2: } $\mathcal{A}(a_2, \sigma, s) = 0$
    ...
    \item \textbf{case 7.2: } $a = a_1 / a_2$ for some $a_1, a_2 \in \textbf{AExp}$ and $(a_1 / a_2)$ has at least one subterm of type \textbf{Float}

    Assume for an induction on the correctness of the compilation of $a_1$ and $a_2$.

        \begin{itemize}
            \item \textbf{subcase 7.2.1: } $\mathcal{A}(a_2, \sigma, s) \neq 0.0$
          ...
          \]
            ...
            \item \textbf{subcase 7.2.2: } $\mathcal{A}(a_2, \sigma, s) = 0.0$
...
\begin{itemize}
    \item \textbf{case 1: } $b = \texttt{true}$ for some $x \in \textbf{Imm} \cap (\textbf{Int} \cup \textbf{Float})$
  ...
  \]
    ...
    \item \textbf{case 2: } $b = \texttt{false}$ for some $x \in \textbf{Var}$
...
Observe that the implication above mentions the correctness of only terminating programs. However, we also claim that the compilation of non-terminating \texttt{SelVeri} programs are also correct, and we will present our arguments in the \textbf{case 6} of the following proof.

\begin{proof}
If $\underline{\sigma}$ is an erroneous program state, then by section \textbf{2.2.4} of this document and the operational semantic rule of high-level \texttt{SelVeri} language $[ \text{err}_{\text{os}} ]$, the above implication is obviously true \footnote{Furthermore, $\sigma = \sigma_f$ and $s = s_f$, in this case.}.  Therefore, from now on, we assume that $\underline{\sigma}$ represents a non-erroneous program state and will use $\sigma$ to denote it. \\

We will prove the correctness of $\mathcal{C}$ via a strong induction on the number of execution steps, by utilizing the fact that sub-statements naturally require a lesser number of execution steps. The correctness of declarations and \texttt{pass} statements establish our base case, since their compilation include only one command.

\begin{itemize}
    \item \textbf{(base) case 1:} $P = (x \;\texttt{:}\; t)$

        \begin{itemize}
            \item \textbf{subcase 1.1:} $t \in \textbf{Type} \land s(x) = \textbf{Void}$
  ...
  \[
      ...
      \end{aligned}
  \]
  \item \textbf{case 4:} $P = (lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]} \;\texttt{:=}\; a)$\\

    Assume that $lst$ is an $m$-dimensional list where each dimension $k$ has length $n_k$ and and $\mathcal{A}(a, \sigma, s) \neq \bot \,\land\, \mathcal{A}(a, \sigma, s) \in ran\mathcal{L}_{s(x)}$, then:

    \[
      \begin{aligned}
      \langle P,\, \sigma,\, s \rangle
      &=\langle lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]} \;\texttt{:=}\; a,\, \sigma,\, s \rangle \\
      &\sigma[lst\texttt{[$\mathcal{A}(i_1, \sigma, s)$]}\dots\texttt{[$\mathcal{A}(i_m, \sigma, s)$]} \mapsto \mathcal{A}(a, \sigma, s)]
  ...
  \[
      \begin{aligned}
      \langle \mathcal{C}(P),\, e,\, \sigma,\, s,\, pc \rangle
      &= \langle \mathcal{C}(lst\texttt{[$i_1$]}\dots\texttt{[$i_m$]} \;\texttt{:=}\; a),\, e,\, \sigma,\, s,\, pc \rangle \\
      &= \langle \mathcal{CA}(a) : \mathcal{CA}(\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m)) :  \texttt{INSERT}\;lst ,\, e,\, \sigma,\, s,\, pc \rangle
      && \text{(definition of $\mathcal{C}$)} \\   
      &\triangleright^\ast \langle \mathcal{CA}(\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m)) :  \texttt{INSERT}\;lst ,\, \mathcal{A}(a, \sigma, s) : e,\, \sigma,\, s,\, pc + l_a \rangle
      && \text{(correctness of $\mathcal{CA}$)} \\ 
      &\triangleright^\ast \langle \texttt{INSERT}\;lst ,\, \mathcal{A}(\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m), \sigma, s) : \mathcal{A}(a, \sigma, s) : e,\, \sigma,\, s,\, pc + l_a + l_I \rangle
      && \text{(correctness of $\mathcal{CA}$)} \\ 
      &\triangleright \langle \epsilon ,\, e,\, \sigma[lst\texttt{[$\mathcal{A}(\mathit{I}(i_1, \dots, i_m ;n_1,\dots,n_m), \sigma, s)$]} \mapsto \mathcal{A}(a, \sigma, s)],\, s,\, pc + l_a + l_I + 1\rangle
      && \text{(operational semantics)} \\
      &= \langle \epsilon ,\, e,\, \sigma[lst\texttt{[$\mathcal{A}(i_1, \sigma, s)$]}\dots\texttt{[$\mathcal{A}(i_m, \sigma, s)$]} \mapsto \mathcal{A}(a, \sigma, s)],\, s,\, pc + l_a + l_I + 1 \rangle
      && \text{(list flattening)} \\ 
      \end{aligned}
  \]
    

    \item \textbf{case 5:} $P = (\texttt{if } b \texttt{ then } S \texttt{ fi})$\\

        \begin{itemize}
            \item \textbf{subcase 5.1:} $\mathcal{B}(b, \sigma, s) = 1$
            ...
            Assume
                ...
                \[
                  ...
                  &\rightarrow \langle S,\, \sigma ,\, s \rangle
                  && ([\text{if}^1_{\text{os}}])   \\
            ...
            \[
              ...
              &= \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{NOOP} ,\, e,\, \sigma,\, s,\, pc \rangle
              && \text{(definition of $\mathcal{C}$)} \\   
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{NOOP} ,\, \mathcal{B}(b, \sigma, s) : e,\, \sigma,\, s,\, pc + l_b \rangle
              && \text{(correctness of $\mathcal{CB}$)} \\ 
              &= \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{NOOP} ,\, 1 : e,\, \sigma,\, s,\, pc + l_b \rangle \\ 
              &\triangleright \langle \mathcal{C}(S) : \texttt{NOOP} ,\, e,\, \sigma,\, s,\, pc + l_b + 1 \rangle
              && \text{(operational semantics)} \\
              &\triangleright^\ast \langle \texttt{NOOP} ,\, e',\, \underline{\sigma}',\, s',\, pc + l_b + l_S + 1 \rangle
              && \text{(induction hypothesis)} \\
              &\triangleright \langle \epsilon ,\, e',\, \underline{\sigma}',\, s',\, pc + l_b + l_S +2 \rangle
              && \text{(operational semantics)} \\ 
              \end{aligned}
          \]


        \item \textbf{subcase 5.2:} $\mathcal{B}(b, \sigma, s) = 0$
            ...
            Assume
                ...
                \[
                  ...
                  &=\langle \texttt{if } b \texttt{ then } S \texttt{ else pass fi},\, \sigma,\, s \rangle 
                  && \text{(semantical equivalance)}\\
                  &\rightarrow \langle \texttt{pass},\, \sigma ,\, s \rangle
                  && [\text{if}^0_{\text{os}}]   \\
                  &\rightarrow \langle \sigma ,\, s \rangle
                  && [\text{pass}_{\text{os}}]   \\
                  \end{aligned}
            ...
            \[
              ...
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{NOOP} ,\, \mathcal{B}(b, \sigma, s) : e,\, \sigma,\, s,\, pc + l_b \rangle
              && \text{(correctness of $\mathcal{CB}$)} \\ 
              &= \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{NOOP} ,\, 0 : e,\, \sigma,\, s,\, pc + l_b \rangle \\  
    ...
    \item \textbf{case 6:} $P = (\texttt{if } b \texttt{ then } S_1 \texttt{ else } S_2 \texttt{ fi})$\\

    \begin{itemize}
            \item \textbf{subcase 6.1:} $\mathcal{B}(b, \sigma, s) = 1$
            ...
            Assume
                ...
                \[
                  ...
                  &\rightarrow \langle S_1,\, \sigma ,\, s \rangle
                  && [\text{if}^1_{\text{os}}]   \\
                  &\rightarrow^\ast \langle \underline{\sigma}' ,\, s' \rangle \\
                  \end{aligned}
              \]
            ...
            \[
              ...
              &= \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{S_2} : \mathcal{C}(S_1) : \texttt{GOTO}\; pc_{\texttt{NOOP}} : \mathcal{C}(S_2) : \texttt{NOOP} ,\, e,\, \sigma,\, s,\, pc \rangle
              && \text{(definition of $\mathcal{C}$)} \\
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{S_2} : \mathcal{C}(S_1) : \texttt{GOTO}\; pc_{\texttt{NOOP}} : \mathcal{C}(S_2) : \texttt{NOOP} ,\, \mathcal{B}(b, \sigma, s) : e,\, \sigma,\, s,\, pc + l_b \rangle
              && \text{(correctness of $\mathcal{CB}$)} \\
              &= \langle \texttt{JZ}\; pc_{S_2} : \mathcal{C}(S_1) : \texttt{GOTO}\; pc_{\texttt{NOOP}} : \mathcal{C}(S_2) : \texttt{NOOP},\, 1 : e,\, \sigma,\, s,\, pc + l_b \rangle \\
              &\triangleright \langle \mathcal{C}(S_1) : \texttt{GOTO}\; pc_{\texttt{NOOP}} : \mathcal{C}(S_2) : \texttt{NOOP},\, e,\, \sigma,\, s,\, pc + l_b + 1 \rangle
              && \text{(operational semantics)} \\
              &\triangleright^\ast \langle \texttt{GOTO}\; pc_{\texttt{NOOP}} : \mathcal{C}(S_2) : \texttt{NOOP},\, e',\, \underline{\sigma}' ,\, s',\, pc + l_b + l_{S_1} + 1 \rangle
              && \text{(induction hypothesis)} \\
        ...
        \item \textbf{subcase 6.2:} $\mathcal{B}(b, \sigma, s) = 0$
            ...
            Assume
                ...
                \[
                  ...
                  \langle P,\, \sigma,\, s \rangle
                  &=\langle \texttt{if } b \texttt{ then } S_1 \texttt{ else } S_2 \texttt{ fi},\, \sigma,\, s \rangle \\
                  &\rightarrow \langle S_2,\, \sigma ,\, s \rangle
                  && [\text{if}^0_{\text{os}}]   \\
                  &\rightarrow^\ast \langle \underline{\sigma}' ,\, s' \rangle \\
                  \end{aligned}
              \]
            ...
            \[
              ...
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{S_2} : \mathcal{C}(S_1) : \texttt{GOTO}\; pc_{\texttt{NOOP}} : \mathcal{C}(S_2) : \texttt{NOOP} ,\, \mathcal{B}(b, \sigma, s) : e,\, \sigma,\, s,\, pc + l_b \rangle
              && \text{(correctness of $\mathcal{CB}$)} \\
              &= \langle \texttt{JZ}\; pc_{S_2} : \mathcal{C}(S_1) : \texttt{GOTO}\; pc_{\texttt{NOOP}} : \mathcal{C}(S_2) : \texttt{NOOP},\, 0 : e,\, \sigma,\, s,\, pc + l_b \rangle \\
              &\triangleright \langle \mathcal{C}(S_2) : \texttt{NOOP},\, e,\, \sigma,\, s,\, pc_{S_2} \rangle
              && \text{(operational semantics)} \\
              &\triangleright^\ast \langle \texttt{NOOP},\, e',\, \underline{\sigma}' ,\, s',\, pc_{S_2} + 1 \rangle
              && \text{(induction hypothesis)} \\
              &\triangleright \langle \epsilon,\, e',\, \underline{\sigma}' ,\, s',\, pc_{S_2} + 2 \rangle
              && \text{(operational semantics)} \\
              \end{aligned}
          \]
    ...
    \item \textbf{case 7:} $P = (\texttt{while } b \texttt{ do } S \texttt{ od})$\\

        \begin{itemize}
            \item \textbf{subcase 7.1:} $\mathcal{B}(b, \sigma, s) = 0$
            ...
            \[
              ...
              &= \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, e,\, \sigma,\, s,\, pc \rangle
              && \text{(definition of $\mathcal{C}$)} \\   
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, \mathcal{B}(b, \sigma, s) : e,\, \sigma ,\, s,\, pc + l_b \rangle
              && \text{(correctness of $\mathcal{CB}$)} \\
              &= \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, 0 : e,\, \sigma ,\, s,\, pc + l_b \rangle \\
              &\triangleright \langle \texttt{NOOP},\, e,\, \sigma ,\, s,,\, pc_{\texttt{NOOP}} \rangle
              && \text{(operational semantics)} \\
              &\triangleright \langle \epsilon,\, e,\, \sigma ,\, s,,\, pc_{\texttt{NOOP}} + 1 \rangle
              && \text{(operational semantics)} \\
          ...
          \item \textbf{subcase 7.2:} $\mathcal{B}(b, \sigma, s) = 1$

          \[
              \begin{aligned}
              \langle P,\, \sigma,\, s \rangle
              &=\langle \texttt{while } b \texttt{ do } S \texttt{ od},\, \sigma,\, s \rangle \\
              &\rightarrow \langle S \texttt{;while } b \texttt{ do } S \texttt{ od},\, \sigma,\, s \rangle
              && [\text{while}^1_{\text{os}}]   \\
          ...
          \[
              ...
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, \mathcal{B}(b, \sigma, s) : e,\, \sigma ,\, s,\, pc + l_b \rangle
              && \text{(correctness of $\mathcal{CB}$)} \\
              &= \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, 1 : e,\, \sigma ,\, s,\, pc + l_b \rangle \\
              &\triangleright \langle \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, e,\, \sigma ,\, s,\, pc + l_b + 1 \rangle
              && \text{(operational semantics)} \\
              &\triangleright^\ast \langle \texttt{GOTO}\; pc_b : \texttt{NOOP},\, e',\, \underline{\sigma}',\, s',\, pc + l_b + l_S + 1 \rangle
              && \text{(induction hypothesis)} \\
              &\triangleright \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, e',\, \underline{\sigma}',\, s',\, pc_b \rangle
              && \text{(operational semantics)} \\
              &= \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \mathcal{C}(S) : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, e',\, \underline{\sigma}',\, s',\, pc \rangle \\
              &= \langle \mathcal{C}(\texttt{while } b \texttt{ do } S \texttt{ od}),\, e',\, \underline{\sigma}',\, s',\, pc \rangle \\
              \end{aligned}
          \]
        \end{itemize}

        However, the proofs above are not enough to establish the correctness of the compiled \texttt{while}-statements, because they only demonstrate one step of iteration. Nonetheless, with a simple induction, we can generalize this argument to claim that at any step of iteration, the compilation is correct, as the proofs above show that at any given program state and scope, the compilation of one iteration is correct. To put it more explicitly, the cases of 0 iterations (\textbf{subcase 6.1}) and 1 iteration (\textbf{subcase 6.2}) are proven to be correct. Assuming that $i$ iterations are compiled correctly, \textbf{subcase 6.2} also prove that $i+1$ iterations are compiled correctly. Ultimaltely, one can certainly say that any terminating \texttt{SelVeri} loop is compiled correctly.

        Until so far, we have omitted non-terminating programs in our correctness proof. But, the only possible source of non-termination of a \texttt{while}-loop is the semantical value of the boolean guard $b = 1$ being a loop invariant. Although we cannot talk about a terminating program state in such a case; at any execution step, we know that the execution thus far has been correct and will continue to be correct.


  \item \textbf{case 8:} $P = \texttt{call}\; f(a_1 \dots, a_n)$
...
\[
  ...
  && (\text{operational semantics})   \\
  &= \langle  \texttt{ENV} : \texttt{DECL}\;t_1\,x_1 : \texttt{STORE}\; x_1 :  \dots : \texttt{DECL}\;t_1\,x_1 : \texttt{STORE}\; x_n : \mathcal{C}(S_f),\, \mathcal{A}(a_1, \sigma, s) : \dots : \mathcal{A}(a_n, \sigma, s) : e,\, \sigma,\, s,\, pc_f,\, (pc+n+1) : ras \rangle
  && (\text{definition of $\mathcal{C}$})   \\
...
Similar to our proof of the correctness of \texttt{while}-statements, this assumes a finite depth of recursion (0, if there is no recursion) as we have assumed termination. Nevertheless, since at each finite step of execution, the correctness of the executed statements is ensured by the proof above, with an argumentation similar to non-terminationg loops, we can conclude that infinite recursive calls of \texttt{SelVeri} are compiled correctly. \\

\footnotetext{Note that we do not cover the case where the execution of $S_f$ leads to an erroneous state, as the last semantic rule in the section \textbf{2.2.4} of this document and [$\text{err}_{\text{os}}$] rule of high-level semantics ensure the correctness of the translation in this case.}

  \item \textbf{case 9:} $P = S_1\texttt{;}S_2$ for some statements $S_1$, $S_2$\\
...
A non-terminating \texttt{SelVeri} program is called \textit{divergent}.  has two possible conditions for non-termination:
...
Nonetheless, we have informally argued the correctness of these divergent programs, in the cases 6 and 7 of the proof in section \textbf{4.2}. Thusfore, one can also claim that the compilation of non-terminating \texttt{SelVeri} programs are also correct. \\
