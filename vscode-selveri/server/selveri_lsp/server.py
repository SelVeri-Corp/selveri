...
def _is_ir_uri(uri: str) -> bool:
    path = unquote(urlparse(uri).path)
    return path.lower().endswith(".svir")
...
def _validate(ls: LanguageServer, uri: str, source: str) -> None:
    ...
    diags = diagnostics.get_diagnostics(source, uri)
    ls.text_document_publish_diagnostics(
...
def on_hover(
    ...
    if _is_ir_uri(params.text_document.uri):
        return None
...
def on_completion(
    ...
    doc = ls.workspace.get_text_document(params.text_document.uri)
    if _is_ir_uri(params.text_document.uri):
        items = completion.get_ir_completions(doc.source, params.position)
    else:
