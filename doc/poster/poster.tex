...
&\mid& \mathtt{if}\ G\ \mathtt{then}\ S^*
[\mathtt{else}\ S^*]\ \mathtt{fi} \\[1pt]
&\mid& \mathtt{while}\ G\ \mathtt{do}\ S^*\ \mathtt{od} \\[3pt]
A &::=& n \mid r \mid x \mid [\overline{A}]
\mid A\oplus A \mid -A
\mid x[A] \mid \mathtt{len}(x) \mid \mathtt{read}() \\[1pt]
&\mid& \mathtt{obtain}(\&x,\{\mathit{Spec}\}) \\[3pt]
G &::=& \mathtt{true} \mid \mathtt{false}
\mid A\bowtie A \mid \mathtt{not}\ G
\mid G\circ G \mid \{\mathit{Spec}\}
\end{array}
\]
\end{mathbox}
