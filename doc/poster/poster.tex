...
\begin{minipage}[c]{0.15\textwidth}
  \centering
  \begin{mathbox}[width=4.8cm, nobeforeafter, halign=center, boxsep=0pt, left=0.3cm, right=0.3cm, top=0.3cm, bottom=0.3cm]
    \color{posterblue}\qrcode[height=4.0cm]{https://github.com/SelVeri-Corp/selveri}
  \end{mathbox}
  
  \vspace{0.2cm}
  {\fontsize{15}{19}\selectfont\color{posterlight} Scan the QR code to checkout\\[0.05cm]SelVeri GitHub repository!\par}
\end{minipage}
\end{tcolorbox}
...
\texttt{x : Int;} \\
\texttt{x := 7 + 9;} \\
\texttt{\{ }$\exists \mathtt{y} \in \mathbb{Z}$\texttt{. x = \&y * \&y \};} \\
\texttt{write(x);}
\end{tabular}
&
&
\begin{tabular}{l}
\texttt{DECL INT x} \\
\texttt{PUSH 9 : PUSH 7 : ADD : STORE x} \\
\texttt{VERI '}$\exists \texttt{y} \in \mathbb{Z}$\texttt{. x = \&y * \&y'} \\
\texttt{LOAD x : WRITE}
\end{tabular}
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{l}
\texttt{x := obtain(\&x,} $\exists$ \texttt{y} $\in$ \texttt{Int. \&y = n*n*n)} \\
\end{tabular}
\end{center}
\end{mathbox}
}
...
\begin{posterbox}[height=25.1cm,compactboxtitle]{Future Work}
{\posterbodyfont
\begin{itemize}
  ...
  \item \textbf{Optimization:} More \textbf{optimized abstract machine} (possibly written in a faster language), dependency-aware LTL verification and definite accept state detection for DFAs, more optimized (possibly incremental) SMT queries, more efficient encoding of runtime configurations, and multi-threading for Verification Engine.
  ...
  \item \textbf{Idiomatic formalization:} SelVeri's formalization, although strong, has been done with pen and paper. A more rigorous mechanized formalization in a \textbf{proof assistant} such as Lean or Isabelle would be a valuable future contribution. 

  \item \textbf{Further extending SelVeri LP:} Logic programming extension to SelVeri language can be extended even further, for example by adding \textbf{when-triggers} which activates at the moment when a given logical specification becomes true.
\end{itemize}
}
\end{posterbox}
...
\begin{posterbox}[height=23.91cm,compactboxtitle]{References}
{\posterbodyfont
\begin{enumerate}
  \setlength{\itemsep}{0.03cm}
