...
To ensure independent history tracking for horizontally isolated blocks, every variable $v$ mapped to \mathtt{Z3} is prefixed with its owner scope's unique identifier (\texttt{scope\_id}). We define the naming function $\mathcal{ZN}$ as:
\[
...
Let $\mathcal{ZS}$ be the mapping function from \texttt{SelVeri} types to \mathtt{Z3} Sorts. Variables are declared in the \texttt{free\_var\_map} according to their type:
...
For a given state mapping variable $v$ to value $val$, assertions are added to the \mathtt{Z3} solver to constrain the free variables. 
...
The mapper recursively translates Abstract Syntax Tree (AST) nodes of the specification into \mathtt{Z3} expressions. We define $\mathcal{ZA}$ for arithmetic expressions, $\mathcal{ZB}$ for boolean expressions, and $\mathcal{ZF}$ for generic FOL formulas.
...
\textit{Note: $\mathit{LF_{Z3}}$ is the \mathtt{Z3} equivalent of the list flattening logic, computed dynamically using dimension length shadows ($\texttt{\_}lst\texttt{\_len\_}d$) from the mapping scope.}
...
\begin{itemize}
    ...
    If the domain is unbounded (\texttt{DomainType}), \mathtt{Z3} native quantifiers are utilized directly:
    \[
    \begin{aligned}
...
\begin{cases}
\langle c ,\, e,\, \sigma,\, s,\, pc + 1 \rangle &\section{Discrete time steps}
￼
...
When the abstract machine encounters a \texttt{STEP} instruction, the verification engine's \texttt{on\_step} hook is invoked[cite: 10]. This momentarily halts standard execution to perform the following temporal maintenance routines in strictly this order:
￼
\begin{enumerate}
    \item \textbf{History Snapshot (for past-LTL):} The engine captures the current \texttt{RuntimeConfiguration} snapshot (comprising the current program state and scope) and appends it to the \texttt{history} array[cite: 10]. It also initializes a new, empty dictionary in \texttt{pltl\_memo} to cache temporal evaluations for this specific step[cite: 10].
    
    \item \textbf{Global Clock Increment:} The internal monotonic clock (\texttt{last\_step}) is incremented by exactly one[cite: 10].
    
    \item \textbf{Automaton Advancement (for future-LTL):} The engine delegates the captured snapshot to \texttt{advance\_future\_obligations}[cite: 10]. It iterates over all pending future obligations residing in the \texttt{fltl\_pending} queue, evaluating their propositional atoms against the snapshot, and advancing their underlying Deterministic Finite Automata (DFA) to their respective next states[cite: 10].
\end{enumerate}
￼
By abstracting the granular execution steps into discrete memory snapshots, the \texttt{STEP} instruction elegantly bridges the gap between the imperative execution model of \texttt{SelVeri} and the strict discrete-time semantics of Linear Temporal Logic. \text{if } \mathcal{M}_{\mathtt{Z3}}(\sigma, s) \models \mathcal{ZF}(\phi)  \\
...
\begin{itemize}
    \item \textbf{Base Case (FOL):} If $\phi$ is a pure FOL formula, it is evaluated directly against the current state snapshot using \mathtt{Z3}:
    \[
...
\begin{enumerate}
    \item \textbf{Base Soundness:} The leaves of the temporal Abstract Syntax Tree (AST) are pure FOL formulae. As argued in \textbf{section 3}, the evaluation of these base FOL propositions using the \texttt{Z3Mapper} and the \mathtt{Z3} solver is sound.
