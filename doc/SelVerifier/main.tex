...
When the abstract machine encounters a \texttt{STEP} instruction, the engine momentarily halts standard execution to perform the following temporal maintenance routines:

\begin{enumerate}
    \item \textbf{Global Clock Increment:} The internal monotonic clock (\texttt{last\_step}) is incremented by exactly one.
    
    \item \textbf{History Snapshot (for past-LTL):} The engine captures a deep copy of the current \texttt{RuntimeConfiguration} (comprising the current program state and scope) and appends it to the \texttt{history} array. This discrete trace serves as the finite backward timeline required for evaluating past-oriented temporal operators.
    
    \item \textbf{Automaton Advancement (for future-LTL):} The engine iterates over all pending future obligations residing in the \texttt{fltl\_pending} queue. The propositional atoms of each obligation are evaluated against the newly recorded snapshot, and their underlying Deterministic Finite Automata (DFA) are advanced to their respective next states.
\end{enumerate}
