...
% Allow extra stretch as a last resort to avoid lines spilling into the margin
\setlength{\emergencystretch}{2em}

\title{SelVeri: Self-Verifying Programming Language \\ High Level Language Syntax and Semantics Document \\ v1.0.0}
...
Integer and real-number literals are given by the following lexical categories:
\[
\begin{array}{rcl}
\langle \mathit{Ident} \rangle &::=& [A\texttt{-}Za\texttt{-}z][A\texttt{-}Za\texttt{-}z0\texttt{-}9\_]^* \\
\langle \mathit{IntLit} \rangle &::=& [0\texttt{-}9]^+ \\
\langle \mathit{RealLit} \rangle &::=& [0\texttt{-}9]^+\texttt{.}[0\texttt{-}9]^+
\end{array}
\]
...
\;\mid\;
\langle \mathit{SpecTemp} \rangle\;\mathtt{Until}\;\langle \mathit{SpecUnary} \rangle \\[2pt]
&&
\mid\;
...
\;\mid\;
\mathtt{Previously}\;\langle \mathit{SpecUnary} \rangle \\[2pt]
&&
\mid\;
...
&&
\mid\;
\mathtt{Forall}\;\langle \mathit{Ident} \rangle\; \mathtt{:}\; \langle \mathit{Domain} \rangle\;\texttt{.}\;\langle \mathit{Spec} \rangle \\[2pt]
&&
\mid\;
...
\;\mid\;
\mathtt{[}\; \langle \mathit{RealLit} \rangle \; \mathtt{,}\; \langle \mathit{RealLit} \rangle \; \mathtt{]} \\[2pt]
&&
\mid\;
\mathtt{(}\; \langle \mathit{RealLit} \rangle \; \mathtt{,}\; \langle \mathit{RealLit} \rangle \; \mathtt{)}
\;\mid\;
\mathtt{[}\; \langle \mathit{RealLit} \rangle \; \mathtt{,}\; \langle \mathit{RealLit} \rangle \; \mathtt{)} \\[2pt]
&&
\mid\;
...
\langle \mathit{Ident} \rangle
\;\mid\;
\langle \mathit{BasicType} \rangle \\[2pt]
&&
\mid\;
...
\footnotetext{This approach might lead to the generation of broken IR code as a non-valid specification is accepted by the parser. However, this is not a problem as the verification module will reject any non-valid specification and the user will be notified about the error.}
...
&::=&
\mathtt{write}\;\langle \mathit{Ident} \rangle
...
\langle \mathit{FuncStmtSeq} \rangle
&::=&
\langle \mathit{FuncStmt} \rangle^* \\[4pt]
...
\langle \mathit{ListAssign} \rangle\texttt{;}
\;\mid\;
\langle \mathit{IOStmt} \rangle \\[4pt]
&& \;\mid\;
\langle \mathit{FuncCall} \rangle\texttt{;}
\;\mid\;
\mathtt{pass}\texttt{;}
...
Let \textbf{Type} be the set of all possible types. For the current implementation of \texttt{SelVeri}, an element of \textbf{Type} can be either \textbf{Int}, \textbf{Real} or a \textbf{List} of any dimension, including one of the former two, which can be interpreted as a dependent type. More formally:
\begin{itemize}
    \item $\textbf{Int} : \textbf{Type}$ 
    \item $\textbf{Real} : \textbf{Type}$ 
    \item $ \textbf{List}: (\Pi{t} : \textbf{BasicType} \, . \,  (\Pi{d} : \textbf{$\mathbb{N}^+$} \, . \,  (\Pi{\vec{n}} : \textbf{$\mathbb{N}^d$} \, . \,  \textbf{Type})))$
\end{itemize} where $\textbf{BasicType} := \{\textbf{Int}, \textbf{Real}\}$. Similarly, we define $\textbf{ListType} := \textbf{Type} \setminus \textbf{BasicType}$ 
...
\mathcal{R}&: \textbf{Imm} \cap \textbf{Real} \rightarrow \mathbb{R} \\
...
\begin{itemize}
    ...
    \item $\mathcal{L}_\textbf{Int} = \mathcal{I}$ and $\mathcal{L}_\textbf{Real} = \mathcal{R}$
\end{itemize}
...
\textbf{Real} & \text{if $s(a') = \textbf{Real}$ for some subterm $a'$ of $a$} \\ 
\end{cases}
\] 
...
Finally, each scope comes with a unique id, which is utilized by \texttt{SelVerifier} to differentiate variables with same name that are declared in different scopes.

\subsubsection{Program states}

Let \textbf{State} be the set of all possible program states and \textbf{Val} \footnote{The reader can notice that $ran\mathcal{I}$, $ran\mathcal{R}$ and $ran\mathcal{L}$ are all subsets of \textbf{Val}.} be the set of all semantic values. Each state is in fact a map from \textbf{Var} and \textbf{Scope} to \textbf{Val}, representing the value of a given variable in the specified scope. Formally,
\[
\forall \sigma \in \textbf{State} \, . \, \sigma: \textbf{Var} \times \textbf{Scope} \rightarrow \textbf{Val}
\]

Each program starts with an empty state, $\tilde{\sigma}$ in which each declared variable has the default value of this type, e.g. 0 for \textbf{Int}, 0.0 for \textbf{Real} and  [0, 0, 0] for \textbf{List[Int, 1, 3]}. If the specific type is unknown in a given context, the default value of the corresponding type is denoted as \textbf{0}.
...
\[
\texttt{function f(x:Int, y:Real, z:List[Real,1,2]) -> Int ::}\; S_\textbf{f} \;\;\texttt{end}
\]
where $S_\textbf{f}$ is some function statement sequence, then 
\[
\Gamma(\texttt{f}) = ((\texttt{x},\, \texttt{Int}), (\texttt{y},\, \texttt{Real}),(\texttt{z},\, \texttt{List[Real,1,2]})),\; \texttt{Int},\; S_\textbf{f})
\]
...
\[
    \iota \in \textbf{InputStream}  \implies \exists n \in \mathbb{N}.\; \iota = [v_0, v_1, v_2, \dots, v_n] \in \textbf{Imm}^{n\footnotemark} 
\]
...
\mathcal{A}(x, \sigma, s, \iota) &= \mathcal{R}(x) \quad \text{if } x \in \textbf{Imm} \land s(x) = \textbf{Real} \\
...
\begin{cases}
    ...
    \bot & \text{if } \mathcal{A}(a_1, \sigma, s, \iota) = \bot \lor \mathcal{A}(a_2, \sigma, s, \iota) = \bot
\end{cases} \\
\mathcal{A}(a_1 / a_2, \sigma, s, \iota)\footnotemark &=
\begin{cases}
...
\end{cases} \\
\mathcal{A}(\text{read}(), \sigma, s, i : \iota) &= \mathcal{V}(i) \quad \text{if } i \in \textbf{Imm} \land s(x) \in \textbf{BasicType}
\end{aligned}
$
\end{tabular}%
}
...
\end{table}
\footnotetext{If $(a_1 / a_2)$ has no subterm of type \textbf{Real}, then the $/$ symbol represents the integer division; otherwise, it represents the division of the field $\mathbb{R}$.}

Although not explicitly stated in the table above, given any binary operation, operators are required to be of the same type (except for the cases of implicit coercion \footnote{An example is multiplying an integer with a real. In such cases, compiler converts the integer to a real with the same value. Conversion of integers to reals is the only possibility for an implicit coercion, as for the current implementation of \texttt{SelVeri}.} handled by the compiler) and can only be of \textit{basic types} \footnote{The only basic types are \textbf{Int} and \textbf{Real} for the current implementation of \texttt{SelVeri}.}. Otherwise, the result of the evaluation is $\bot$. The same rules apply to the next section (boolean expressions) as well.
...
\begin{table}[h!]
\centering
\renewcommand{\arraystretch}{1.6} % increase vertical spacing
...
\begin{table}[!ht]
\centering
\renewcommand{\arraystretch}{1.7} % Significantly increased global line spacing
\makebox[\textwidth][c]{%
\begin{tabular}{|r l|}
\hline
...
\newpage
\newpage
\section{Determinism of \texttt{SelVeri}}

In order to rigorously establish that \texttt{SelVeri} is a deterministic language, we must prove that its small-step operational semantics transition relation ($\rightarrow$) is a partial function.

\textbf{Theorem (Determinism):} For any statement $S$, state $\sigma$, and scope $s$, if $\langle S, \sigma, s \rangle \rightarrow \gamma_1$ and $\langle S, \sigma, s \rangle \rightarrow \gamma_2$, then $\gamma_1 = \gamma_2$.

\textbf{Proof:} The proof proceeds by structural induction on the statement $S$. Before evaluating the statements, we first establish the determinism of expressions.

\textit{Step 1: Determinism of Expressions} \\
According to our semantics, the arithmetic valuation function $\mathcal{A}$ is defined as a deterministic mathematical function mapping a tuple of (\textbf{AExp}, \textbf{State}, \textbf{Scope}, \textbf{InputStream}) to exactly one value in \textbf{Val} (Table \ref{tab:arith-semantics}). Similarly, the boolean valuation function $\mathcal{B}$ maps to exactly one value in $\{0, 1, \bot\}$ (Table \ref{tab:boolean-semantics}). Because these evaluations are strictly defined as functions, they inherently yield exactly one result for any given configuration.

\textit{Step 2: Mutual Exclusivity of Statement Rules} \\
We proceed by structural induction on $S$, showing that for any given configuration, at most one operational semantic rule can apply (Table \ref{tab:while-semantics}).
\begin{itemize}
    \item \textbf{Control Flow (\texttt{if} and \texttt{while}):} The transition rules for \texttt{if} ($[\text{if}^{1}_{\text{os}}]$, $[\text{if}^{0}_{\text{os}}]$, $[\text{if}^{\bot}_{\text{os}}]$) and \texttt{while} ($[\text{while}^{1}_{\text{os}}]$, $[\text{while}^{0}_{\text{os}}]$, $[\text{while}^{\bot}_{\text{os}}]$) depend strictly on the evaluation of $\mathcal{B}(b, \sigma, s)$. Since $\mathcal{B}$ deterministically evaluates to exactly $1$, $0$, or $\bot$, these premises are mutually exclusive.
    \item \textbf{Assignments:} The basic assignment rules ($[\text{ass}_{\text{os}}]$ and $[\text{ass}^{\bot}_{\text{os}}]$) possess mutually exclusive premises. $[\text{ass}_{\text{os}}]$ requires $\mathcal{A}(a, \sigma, s) \neq \bot \land \mathcal{A}(a, \sigma, s) \in ran\mathcal{L}_{s(x)}$. The rule $[\text{ass}^{\bot}_{\text{os}}]$ fires if and only if this exact condition is false ($\mathcal{A}(a, \sigma, s) = \bot \lor \mathcal{A}(a, \sigma, s) \notin ran\mathcal{L}_{s(x)}$). The same mutual exclusivity applies to list assignments.
    \item \textbf{Declarations:} The declaration rules ($[\text{decl}_{\text{os}}]$ and $[\text{decl}^{\bot}_{\text{os}}]$) rely on mutually exclusive type and scope checks. $[\text{decl}_{\text{os}}]$ applies if $t \in \textbf{BasicType} \land s(x) = \textbf{Void}$, whereas $[\text{decl}^{\bot}_{\text{os}}]$ acts as the deterministic fallback if $t \notin \textbf{Type} \lor s(x) \neq \textbf{Void}$.
    \item \textbf{Error States:} If a state is already erroneous ($\hat{\sigma}$), the catch-all error rule $[\text{err}_{\text{os}}]$ deterministically transitions the program into $\hat{\sigma}$. Furthermore, as stated previously, any configuration absent from the operational semantics table also deterministically transitions into $\hat{\sigma}$.
\end{itemize}

Because the expressions are deterministic and the transition rules for statements are mutually exclusive, it is impossible for more than one operational semantic rule to apply to any given configuration. Therefore, $\gamma_1 = \gamma_2$, and the semantics of \texttt{SelVeri} are strictly deterministic. $\blacksquare$

\newpage
\section{Turing-completeness of \texttt{SelVeri}}

To formally establish the Turing-completeness of \texttt{SelVeri}, we demonstrate that the language can compute all $\mu$-recursive (general recursive) functions. By the Church-Turing thesis, any computational model capable of representing the basic functions and closed under composition, primitive recursion, and minimization is Turing-complete. 

\textbf{Theorem:} Every general recursive function can be computed by a valid \texttt{SelVeri} program.

\textbf{Proof:} We show that \texttt{SelVeri} constructs, underpinned by the operational semantics defined in \textbf{section 2}, are sufficient to implement the foundational elements of $\mu$-recursive functions. We utilize the \texttt{Int} basic type, which semantically maps to the unbounded infinite set of integers ($\mathbb{Z}$) via the valuation function $\mathcal{I}$. \\

\textit{1. The Basic Functions}
\begin{itemize}
    \item \textbf{Zero Function} $Z(x) = 0$: Supported natively by returning the integer immediate $0$.
    \begin{verbatim}
function Z(x : t) -> Int ::
    return 0;
end
    ...
    \item \textbf{Successor Function} $S(x) = x + 1$: Supported by the arithmetic valuation function $\mathcal{A}(\texttt{$x$ + 1}, \sigma, s, \iota)$.
    \begin{verbatim}
function S(x: Int) -> Int ::
    ...
    \item \textbf{Projection Function} $P_i^k(x_1, \dots, x_k) = x_i$: Supported natively by the parameter passing and scope rules ($s(x_i) = \textbf{Int}$) of \texttt{SelVeri} function declarations. For example, $P_i^k(x_1, \dots, x_k) = x_i$ is implemented as:
    \begin{verbatim}
function P_i_k(x1 : t_i, ..., x_k : t_k) -> t_i ::
    ...
    Similarly, list index accessing (\texttt{lst[i]}) can be interpreted as another example of projection function in \texttt{SelVeri}.
\end{itemize}

\newpage
\textit{2. Closure Under Composition} \\
Given a function $h(\vec{x}) = f(g_1(\vec{x}), \dots, g_m(\vec{x}))$, \texttt{SelVeri} natively handles composition through sequential function calls and the dynamically-typed \texttt{retvar} mechanism. Each inner function $g_i$ can be evaluated using a $\mathtt{call}\; g_i(\dots)$ statement. By the $[\text{call}_{\text{os}}]$ and $[\text{ret}_{\text{os}}]$ rules, the result is safely stored in the \texttt{retvar} scope, assigned to an intermediate variable, and subsequently passed as an argument to the outer function $f$. For example, a composition $h(x_1, \dots x_n) = f(g_1(x^1_1 \dots x_{m_1}^1), \dots g_k(x^k_1 \dots x_{m_k}^k))$ is implemented as:
\begin{verbatim}
...
\textit{3. Closure Under Primitive Recursion} \\
A function defined by primitive recursion takes the form:
\begin{align*}
h(\vec{x}, 0) &= f(\vec{x}) \\
h(\vec{x}, y+1) &= g(\vec{x}, y, h(\vec{x}, y))
\end{align*}
This is realized iteratively via \texttt{SelVeri}'s $\mathtt{while}$ loop. Because the \texttt{Int} type models unbounded integers natively without overflow, we can simulate the recursive steps from $0$ up to $y$:
\begin{verbatim}
...
The loop's structural soundness is strictly guaranteed by the mutually exclusive $[\text{while}^1_{\text{os}}]$ and $[\text{while}^0_{\text{os}}]$ semantic rules evaluating $\mathcal{B}(\mathtt{counter} < y, \sigma, s)$. \\
...
The base case of \texttt{y = 0} and strict decrease of \texttt{y} in non-base cases ensure the completion of the function call.


\textit{4. Closure Under Minimization ($\mu$-operator)} \\
The minimization operator $\mu y [f(\vec{x}, y) = 0]$ finds the smallest $y$ such that $f(\vec{x}, y) = 0$. This unbounded search introduces the potential for partial functions (non-halting programs), a strict requirement for full Turing-completeness. In \texttt{SelVeri}, this translates to an unbounded $\mathtt{while}$ loop checking an arithmetic evaluation interpreted as a boolean:
\begin{verbatim}
...
\end{verbatim}
By the operational semantics of boolean expressions established in Table 2, the $\mathtt{while}$ condition evaluates the equality constraint $\mathcal{B}(\mathtt{not}\; (\mathtt{res} \;\mathtt{=}\; 0), \sigma, s)$, causing the loop to terminate and return $y$ if and only if such a $y$ exists.

\textbf{Conclusion:} Because \texttt{SelVeri} represents the basic functions and is closed under composition, primitive recursion, and unbounded minimization using its explicitly defined structural operational semantics, it is capable of computing any $\mu$-recursive function. Thus, \texttt{SelVeri} is Turing-complete. $\blacksquare$
