...
function S(x: Int) -> Int ::
    ...
    \item \textbf{Projection Function} $P_i^k(x_1, \dots, x_k) = x_i$: Supported natively by the parameter passing and scope rules ($s(x_i) = \textbf{Int}$) of \texttt{SelVeri} function declarations. For example, $P_2^3(x_1, x_2, x_3) = x_2$ is implemented as:
    \begin{verbatim}
function P_2_3(x1: Int, x2: Int, x3: Int) -> Int ::
    return x2;
end
    \end{verbatim}
...
Given a function $h(\vec{x}) = f(g_1(\vec{x}), \dots, g_m(\vec{x}))$, \texttt{SelVeri} natively handles composition through sequential function calls and the dynamically-typed \texttt{retvar} mechanism. Each inner function $g_i$ can be evaluated using a $\mathtt{call}\; g_i(\dots)$ statement. By the $[\text{call}_{\text{os}}]$ and $[\text{ret}_{\text{os}}]$ rules, the result is safely stored in the \texttt{retvar} scope, assigned to an intermediate variable, and subsequently passed as an argument to the outer function $f$. For example, a composition $h(x) = f(g_1(x), g_2(x))$ is implemented as:
\begin{verbatim}
...
Note that in the above example the arity of $g$ is assumed to be 1; however, by increasing the number of arguments, any finite arity $k$ can work.  \\
