...
\title{SelVerIR: Self-Verifying Programming Language \\ Intermediate Representation Syntax and Semantics Document \\ v1.0.0}
...
\langle \mathit{basic\_type} \rangle &::=& \texttt{INT} \;\mid\; \texttt{REAL}\\[6pt]

\langle \mathit{list\_type} \rangle &::=& \texttt{LIST[INT]} \;\mid\; \texttt{LIST[REAL]}  \\[6pt]
...
\langle \mathit{real} \rangle &::=& [0\texttt{-}9]^+\texttt{.}[0\texttt{-}9]^+ \\[6pt]

\langle \mathit{imm} \rangle &::=& \langle \mathit{int} \rangle \;\mid\; \langle \mathit{real} \rangle \\[6pt]
...
\large
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} >{\(}l<{\)} @{\(\;\triangleright\;\)} >{\(}l<{\)} @{}}
\hline\hline

\langle \texttt{DECL}\;t\, x : c ,\, e,\, \sigma,\, s,\, pc \rangle & 
\begin{cases}
\langle c,\, e,\, \sigma[x \mapsto \textbf{0}],\, s[x \mapsto t],\, pc+1 \rangle \\ \qquad \text{if } x \in \textbf{Var} \;\land\; t \in \{\texttt{INT}, \texttt{REAL}\} \\ \qquad \;\land\; s(x) = \textbf{Void}\\
\end{cases} \\

\langle \texttt{LDECL}\;t\,x : c ,\, n : e,\, \sigma,\, s,\, pc \rangle & 
\begin{cases}
\langle c,\, e,\, \sigma[x \mapsto \textbf{0}],\, s[x \mapsto \texttt{LIST[INT, $n$]}],\, pc+1 \rangle \\ \qquad \text{if } x \in \textbf{Var} \;\land\; t = \texttt{LIST[INT]}] \\ \qquad \;\land\; s(x) = \textbf{Void}\\

\langle c,\, e,\, \sigma[x \mapsto \textbf{0}],\, s[x \mapsto \texttt{LIST[REAL, $n$]}],\, pc+1 \rangle \\ \qquad \text{if } x \in \textbf{Var} \;\land\; t = \texttt{LIST[REAL]}] \\ \qquad \;\land\; s(x) = \textbf{Void}\\
...
\langle c,\, \sigma(x,s) : e,\, \sigma,\, s,\, pc+1 \rangle & \\ \qquad \text{if } s(x) \in \{\texttt{INT}, \texttt{REAL}\} \land n=1 & \\ \qquad \land\; v_1 \in \mathbb{R} \land x \in \textbf{Var} \\[2pt]
{\begin{array}[t]{@{}l@{}}\langle c,\, n : \sigma(x\texttt{[0]},s) : \dots \\ \quad : \sigma(x\texttt{[n-1]},s): e,\, \sigma,\, s,\, pc+1 \rangle\end{array}} & \\ \qquad \text{if } s(x) = \texttt{LIST[$t, n$]} \land x \in \textbf{Var} \\[2pt]
\langle c,\, e,\, \hat{\sigma},\, s,\, pc+1 \rangle \qquad \text{otherwise}
\end{cases} \\


{\begin{array}[t]{@{}l@{}}\langle \texttt{STORE}\; x : c,\, \\ \quad v_0 : v_1 : \dots : v_n : e,\, \\ \quad \sigma,\, s,\, pc \rangle\end{array}} & \begin{cases}
\langle c,\, e,\, \sigma[x \mapsto v_0],\, s,\, pc+1 \rangle & \\ \qquad \text{if } s(x) \in \{\texttt{INT}, \texttt{REAL}\} \land n=0 & \\ \qquad \land\; v_0 \in \mathbb{R} \land x \in \textbf{Var} \\[2pt]
\langle c,\, e,\, \sigma[x \mapsto [v_1, \dots, v_n]],\, s,\, pc+1 \rangle & \\ \qquad \text{if } s(x) = \texttt{LIST[$t, v_0$]} & \\ \qquad \land\; v_1, \dots v_n \in \mathbb{R} \land x \in \textbf{Var} \\[2pt]
\langle c,\, e,\, \hat{\sigma},\, s,\, pc+1 \rangle \qquad \text{otherwise} \\
\end{cases}\\
...
\langle \texttt{rDIV} : c ,\, r_1 : r_2 : e,\, \sigma,\, s,\, pc \rangle & \begin{cases} 
...
\langle c,\, 0 : e,\, \sigma,\, s,\, pc+1 \rangle & \text{if } s(x) \in \{\texttt{INT}, \texttt{REAL}\} \\
{\begin{array}[t]{@{}l@{}}\langle c,\, \mathcal{A}(\texttt{\_$lst$\_len\_1}, \sigma, s) : e,\, \\ \quad \sigma,\, s,\, pc+1 \rangle\end{array}} & \text{if } s(x) = \texttt{LIST[t, $n$]} \\
\langle c,\, \bot : e,\, \hat{\sigma},\, s,\, pc+1 \rangle & \text{otherwise}
\end{cases} \\
...
{\begin{array}[t]{@{}l@{}}\langle \texttt{CALL}\; f : c ,\, \\ \quad e,\, \sigma,\, s,\, pc,\, ras \rangle\end{array}} &
{\renewcommand{\arraystretch}{1.0}
\begin{array}[t]{@{}l@{}}
\langle P[pc_f:],\, e,\, \sigma,\, s,\, pc_f,\, (pc+1) : ras \rangle \\
\quad \text{if } f \in \textbf{Func} \land \Gamma(f) \neq \emptyset
\end{array}} \\

\langle \texttt{CSCOPE} : c ,\, e,\, \sigma,\, s,\, pc \rangle &
{\renewcommand{\arraystretch}{1.0}
\begin{array}[t]{@{}l@{}}
\langle c,\, e,\, \tilde{\sigma_c},\, \tilde{s_c},\, pc+1 \rangle \\
\quad \text{where $\sigma$ and $s$ are the parents of $\sigma_c$ and $s_c$}
\end{array}}  \\

\langle \texttt{PSCOPE} : c ,\, e,\, \sigma_c,\, s_c,\, pc \rangle &
{\renewcommand{\arraystretch}{1.0}
\begin{array}[t]{@{}l@{}}
\langle c,\, e,\, \sigma,\, s,\, pc+1 \rangle \\
\quad \text{where $\sigma$ and $s$ are the parents of $\sigma_c$ and $s_c$}
\end{array}}  \\

{\begin{array}[t]{@{}l@{}}\langle \texttt{FUNCENV}\; f, t_r : c ,\, \\ \quad e,\, \sigma,\, s,\, pc \rangle\end{array}} &
{\renewcommand{\arraystretch}{1.0}
\begin{array}[t]{@{}l@{}}
\langle c,\, e,\, \tilde{\sigma}',\, \tilde{s}',\, pc+1 \rangle \\
\quad \text{where $f$ names the current function} \\
\quad \text{and $\sigma$ and $s$ is of the caller}
\end{array}}  \\

{\begin{array}[t]{@{}l@{}}\langle \texttt{RET} : c ,\, v : e,\, \\ \quad \sigma',\, s',\, pc ,\, ra : ras \rangle\end{array}} &
{\renewcommand{\arraystretch}{1.0}
\begin{array}[t]{@{}l@{}}
\langle P[ra:],\, e,\, \sigma[\texttt{retvar} \mapsto v],\, \\
\quad s[\texttt{retvar} \mapsto s'(v)],\, ra ,\, ras \rangle \\
\quad \text{if the return-type of the function called} \\
\quad \text{is equal to $s'(v)$} \\
\quad \text{and $\sigma$ and $s$ is of the caller}
\end{array}} \\
...
\text{if $i \in \textbf{Imm} \cap \{\textbf{Int} \cup \textbf{Real}\}$}
\end{array}} \\
...
\begin{itemize}
    ...
    \item \textbf{Arithmetic and Boolean Commands:} Instructions like \texttt{iDIV} and \texttt{rDIV} branch on whether the divisor is zero or non-zero ($z_2 = 0$ vs. $z_2 \neq 0$). Boolean commands like \texttt{AND}, \texttt{OR}, and \texttt{NEG} branch on explicit, mutually exclusive truth values ($1$ or $0$) popped from the stack.
...
= \footnote{Because $\mathit{LF}$ constructs an arithmetic expression rather than computing a raw integer, the $\Sigma$ and $\Pi$ symbols here denote the syntactic generation of chained \texttt{+} and \texttt{*} expressions}
...
\begin{cases}
    ...
    \Pi_{i=1}^d{a_i} & \text{if } t = \textbf{List}[t', d, \vec{a}] \text{ for some } t' \in \textbf{BasicType}
\end{cases}
\end{aligned}
...
\mathcal{CA}(a_2): \mathcal{CA}(a_1) : \texttt{iDIV} & \parbox[t]{0.35\textwidth}{\raggedright  if $(a_1 / a_2)$ has no subterm of type \footnotemark \textbf{Real}} \\[3ex]
\mathcal{CA}(a_2): \mathcal{CA}(a_1) : \texttt{rDIV} & \parbox[t]{0.35\textwidth}
{\raggedright  if $(a_1 / a_2)$ has at least one subterm of type \textbf{Real}} \\[1ex]
\end{cases}
...
\\ \quad \texttt{DECL INT}\;\texttt{\_$lst$\_len\_1} : \mathcal{CA}(a_1) : \texttt{STORE}\;\texttt{\_$lst$\_len\_1} : \dots
\\ \quad : \texttt{DECL INT}\;\texttt{\_$lst$\_len\_$d$} : \mathcal{CA}(a_d) : \texttt{STORE}\;\texttt{\_$lst$\_len\_$d$} : \mathcal{CA}(\texttt{size\_IR(} \textbf{List[$t,d,\vec{a}$]} \texttt{)} ) : \texttt{LDECL LIST[$t$]}\;lst
\\[1.5ex]
...
\\ \quad \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{CSCOPE}_2} : \texttt{CSCOPE} : \mathcal{C}(S_1) : \texttt{PSCOPE} : \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \texttt{PSCOPE} : \texttt{NOOP}
\\[1.5ex]

\mathcal{C}(\texttt{while } b \texttt{ do } S \texttt{ od}) =
\\ \quad \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \texttt{PSCOPE} : \texttt{GOTO}\; pc_b : \texttt{NOOP}
\\[1.5ex]
...
\begin{itemize}
    \item \textbf{case 1: } $a = v$ for some $v \in \textbf{Imm} \cap (\textbf{Int} \cup \textbf{Real})$
    ...
    \[
      \begin{aligned}
        ...
        && \text{(definition of } \mathcal{CA}\text{)} \\
        ...
        && \text{(definition of } \mathcal{A}\text{)} \\
  ...
  Suppose $\mathit{LF}(lst; i_1, \dots, i_d)) = j$ for some $j \in \textbf{AExp}$ and assume for an induction on the correctness of the compilation of $j$. 
    ...
    \[
      \begin{aligned}
        ...
        && \text{(definition of } \mathit{LF}\text{)} \\
  ...
  \]
    ...
    \item \textbf{case 11.1: } $a = a_1 / a_2$ for some $a_1, a_2 \in \textbf{AExp}$ and $(a_1 / a_2)$ has no subterm of type \textbf{Real}
    ...
    \item \textbf{case 11.2: } $a = a_1 / a_2$ for some $a_1, a_2 \in \textbf{AExp}$ and $(a_1 / a_2)$ has at least one subterm of type \textbf{Real}
        ...
        \begin{itemize}
            \item \textbf{subcase 11.2.1: } $\mathcal{A}(a_2, \sigma, s) \neq 0.0$
              ...
              \begin{aligned}
                &\langle  \mathcal{CA}(a) : c,\, e,\, \sigma,\, s,\, pc \rangle \\
                &= \langle \mathcal{CA}(a_1 / a_2) : c,\, e,\, \sigma,\, s,\, pc \rangle \\
                &= \langle \mathcal{CA}(a_2) : \mathcal{CA}(a_1) : \texttt{rDIV} :  c,\, e,\, \sigma,\, s,\, pc \rangle 
                && \text{(definition of } \mathcal{CA}\text{)} \\
                &\triangleright^{\ast} \langle \mathcal{CA}(a_1) : \texttt{rDIV} :  c,\, \mathcal{A}(a_2, \sigma, s) : e,\, \sigma,\, s,\, pc + l_2 \rangle 
                && \text{(induction hypothesis 2)} \\
                &\triangleright^{\ast} \langle \texttt{rDIV} :  c,\, \mathcal{A}(a_1, \sigma, s) : \mathcal{A}(a_2, \sigma, s) : e,\, \sigma,\, s,\, pc + l_2 + l_1 \rangle
                && \text{(induction hypothesis 1)} \\
                &\triangleright \langle c,\, \mathcal{A} (a_1, \sigma, s) /  \mathcal{A}(a_2, \sigma, s) : e,\, \sigma,\, s,\, pc + l_2 + l_1 + 1 \rangle
                && \text{(operational semantics)} \\
                &= \langle c,\, \mathcal{A}(a_1 / a_2, \sigma, s) : e,\, \sigma,\, s,\, pc + l_2 + l_1 + 1 \rangle
          ...
          \]
            ...
            \[
              \begin{aligned}
                ...
                &\triangleright^{\ast} \langle \mathcal{CA}(a_1) : \texttt{rDIV} :  c,\, \mathcal{A}(a_2, \sigma, s) : e,\, \sigma,\, s,\, pc + l_2 \rangle 
                && \text{(induction hypothesis 2)} \\
                &= \langle \mathcal{CA}(a_1) : \texttt{rDIV} :  c,\, 0.0 : e,\, \sigma,\, s,\, pc + l_2 \rangle \\
                &\triangleright^{\ast} \langle \texttt{rDIV} :  c,\, \mathcal{A}(a_1, \sigma, s) : 0.0 : e,\, \sigma,\, s,\, pc + l_2 + l_1 \rangle
                && \text{(induction hypothesis 1)} \\
                &\triangleright \langle c,\, \bot : e,\, \hat{\sigma},\, s,\, pc + l_2 + l_1 + 1 \rangle 
                && \text{(operational semantics)} \\
                &= \langle c,\, \mathcal{A}(a_1 / a_2, \sigma, s) : e,\, \hat{\sigma},\, s,\, pc + l_2 + l_1 + 1 \rangle
...
\begin{itemize}
    ...
    \[
      \begin{aligned}
        ...
        && \text{(definition of } \mathcal{CB}\text{)} \\
        ...
        && \text{(definition of } \mathcal{B}\text{)} \\
     ...
     \begin{itemize}
         ...
         \[
          \begin{aligned}
            ...
            && \text{(correctness of } \mathcal{CA}\text{)} \\
...
\begin{itemize}
    \item \textbf{(base) case 1:} $P = (x \;\texttt{:}\; t)$
        ...
        \begin{itemize}
          ...
          \[
              ...
              && \text{(definition of } \mathcal{C}\text{)} \\   
    ...
    Let $\sigma' = \sigma[\texttt{\_$lst$\_len\_i} \mapsto \mathcal{A}(a_i, \sigma, s)]$ and $s' = s[\texttt{\_$lst$\_len\_i} \mapsto \textbf{Int}]$ for $ i \in \{1,\dots,d\}$ 
      ...
      and
          ...
          \[
              ...
              &= \langle \texttt{DECL INT}\;\texttt{\_$lst$\_len\_1} : \mathcal{CA}(a_1) : \texttt{STORE}\;\texttt{\_$lst$\_len\_1} : \dots \\
              &\quad : \texttt{DECL INT}\;\texttt{\_$lst$\_len\_$d$} : \mathcal{CA}(a_d) : \texttt{STORE}\;\texttt{\_$lst$\_len\_$d$} \\
              &\quad : \mathcal{CA}(\texttt{size\_IR(} \textbf{List[$t,d,\vec{a}$]} \texttt{)} ) : \texttt{LDECL LIST[$t$]}\;lst,\, e,\, \sigma,\, s,\, pc \rangle
              && \text{(definition of } \mathcal{C}\text{)} \\
              &\triangleright^\ast \langle \mathcal{CA}(\texttt{size\_IR(} \textbf{List[$t, a$]} \texttt{)} ) : \texttt{LDECL LIST[$t$]}\;lst,\, e,\, \sigma',\, s',\, pc + l \rangle
              && \text{(operational semantics))} \\
              &\triangleright \langle \texttt{LDECL LIST[$t$]}\;lst,\, \mathcal{A}(\texttt{size\_IR(} \textbf{List[$t, d, \vec{a}$]} \texttt{)}, \sigma, s) : e,\, \\
              &\quad \sigma[\texttt{\_$lst$\_len\_1} \mapsto \mathcal{A}(a, \sigma, s)],\, s[\texttt{\_$lst$\_len\_1} \mapsto \texttt{INT}],\, pc + l + l_a \rangle
              && \text{(correctness of } \mathcal{CA}\text{)} \\
              &\triangleright \langle \epsilon,\, e,\, \sigma'[lst \mapsto \textbf{0}],\, \\
              &\quad s'[lst \mapsto \texttt{LIST[$t, \mathcal{A}(\texttt{size\_IR(} \textbf{List[$t, d, \vec{a}$]} \texttt{)}, \sigma, s)$]}],\, pc + l + l_a + 1\rangle
              && \text{(operational semantics)} \\
  ...
  \[
      ...
      && \text{(definition of } \mathcal{C}\text{)} \\   
      &\triangleright^\ast \langle \mathcal{CA}(\mathit{LF}(lst; i_1, \dots, i_d)) :  \texttt{LSTORE}\;lst ,\, \mathcal{A}(a, \sigma, s) : e,\, \\
      &\quad \sigma,\, s,\, pc + l_a \rangle
      && \text{(correctness of } \mathcal{CA}\text{)} \\
      &\triangleright^\ast \langle \texttt{LSTORE}\;lst ,\, \mathcal{A}(\mathit{LF}(lst\;; i_1, \dots, i_d), \sigma, s) : \mathcal{A}(a, \sigma, s) : e,\, \\
      &\quad \sigma,\, s,\, pc + l_a + l_I \rangle
      && \text{(correctness of } \mathcal{CA}\text{)} \\
      &\triangleright \langle \epsilon ,\, e,\, \sigma[lst\texttt{[$\mathcal{A}(\mathit{LF}(lst\;; i_1, \dots, i_d), \sigma, s)$]} \mapsto \mathcal{A}(a, \sigma, s)],\, \\
      &\quad s,\, pc + l_a + l_I + 1\rangle
      && \text{(operational semantics)} \\
      &= \langle \epsilon ,\, e,\, \sigma[lst\texttt{[$\mathcal{A}(i_1, \sigma, s)$]}\dots\texttt{[$\mathcal{A}(i_d, \sigma, s)$]} \mapsto \mathcal{A}(a, \sigma, s)],\, \\
      &\quad s,\, pc + l_a + l_I + 1 \rangle
      && \text{(list flattening)} \\
      \end{aligned}
  \]
    ...
    \item \textbf{case 7:} $P = (\texttt{if } b \texttt{ then } S \texttt{ fi})$\\
        ...
        \begin{itemize}
            ...
            \[
              ...
              &= \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \texttt{PSCOPE} :\texttt{NOOP} ,\, \\
              &\quad e,\, \sigma,\, s,\, pc \rangle
              && \text{(definition of } \mathcal{C}\text{)} \\
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \texttt{PSCOPE} :\texttt{NOOP} ,\, \\
              &\quad \mathcal{B}(b, \sigma, s) : e,\, \sigma,\, s,\, pc + l_b \rangle
              && \text{(correctness of } \mathcal{CB}\text{)} \\
    ...
    \begin{itemize}
            ...
            \[
              ...
              &= \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{CSCOPE}_2} : \texttt{CSCOPE} : \mathcal{C}(S_1) : \texttt{PSCOPE} : \\
              &\quad \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \\
              &\quad \texttt{PSCOPE} : \texttt{NOOP} ,\, e,\, \sigma,\, s,\, pc \rangle
              && \text{(definition of } \mathcal{C}\text{)} \\
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{\texttt{CSCOPE}_2} : \texttt{CSCOPE} : \mathcal{C}(S_1) : \texttt{PSCOPE} : \\
              &\quad \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \\
              &\quad \texttt{PSCOPE} : \texttt{NOOP} ,\, \mathcal{B}(b, \sigma, s) : e,\, \sigma,\, s,\, pc + l_b \rangle
              && \text{(correctness of } \mathcal{CB}\text{)} \\
              &= \langle \texttt{JZ}\; pc_{\texttt{CSCOPE}_2} : \texttt{CSCOPE} : \mathcal{C}(S_1) : \texttt{PSCOPE} : \\
              &\quad \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \\
              &\quad \texttt{PSCOPE} : \texttt{NOOP},\, 1 : e,\, \sigma,\, s,\, pc + l_b \rangle \\
              &\triangleright \langle \texttt{CSCOPE} : \mathcal{C}(S_1) : \texttt{PSCOPE} : \\
              &\quad \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \\
              &\quad \texttt{PSCOPE} : \texttt{NOOP},\, e,\, \sigma,\, s,\, pc + l_b + 1 \rangle
              && \text{(operational semantics)} \\
              &\triangleright \langle \mathcal{C}(S_1) : \texttt{PSCOPE} : \\
              &\quad \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \\
              &\quad \texttt{PSCOPE} : \texttt{NOOP},\, e,\, \tilde{\sigma_c},\, \tilde{s_c},\, pc + l_b + 2 \rangle
              && \text{(operational semantics)} \\
              &\triangleright^\ast \langle \texttt{PSCOPE} : \\
              &\quad \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \\
              &\quad \texttt{PSCOPE} : \texttt{NOOP},\, e',\, \sigma'_c ,\, s'_c,\, pc + l_b + l_{S_1} + 2 \rangle
              && \text{(induction hypothesis)} \\
              &\triangleright \langle \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \\
              &\quad \texttt{PSCOPE} : \texttt{NOOP},\, e',\, \underline{\sigma}' ,\, s',\, pc + l_b + l_{S_1} + 3 \rangle
              && \text{(induction hypothesis)} \\
        ...
        \item \textbf{subcase 8.2:} $\mathcal{B}(b, \sigma, s) = 0$
            ...
            \[
              ...
              &= \langle \texttt{JZ}\; pc_{\texttt{CSCOPE}_2} : \texttt{CSCOPE} : \mathcal{C}(S_1) : \texttt{PSCOPE} : \\
              &\quad \texttt{GOTO}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S_2) : \\
              &\quad \texttt{PSCOPE} : \texttt{NOOP},\, 0 : e,\, \sigma,\, s,\, pc + l_b \rangle \\
    ...
    \item \textbf{case 9:} $P = (\texttt{while } b \texttt{ do } S \texttt{ od})$\\
        ...
        \begin{itemize}
            ...
            \[
              ...
              &= \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \\
              &\quad \texttt{PSCOPE} : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, e,\, \sigma,\, s,\, pc \rangle
              && \text{(definition of } \mathcal{C}\text{)} \\   
              &\triangleright^\ast \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \\
              &\quad \texttt{PSCOPE} : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, \mathcal{B}(b, \sigma, s) : e,\, \sigma ,\, s,\, pc + l_b \rangle
              && \text{(correctness of } \mathcal{CB}\text{)} \\
              &= \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \\
              &\quad \texttt{PSCOPE} : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, 0 : e,\, \sigma ,\, s,\, pc + l_b \rangle \\
              &\triangleright \langle \texttt{NOOP},\, e,\, \sigma ,\, s,,\, pc_{\texttt{NOOP}} \rangle
              && \text{(operational semantics)} \\
              &\triangleright \langle \epsilon,\, e,\, \sigma ,\, s,,\, pc_{\texttt{NOOP}} + 1 \rangle
              && \text{(operational semantics)} \\
          ...
          \[
              ...
              && \text{(correctness of } \mathcal{CB}\text{)} \\
              &= \langle \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \\
              &\quad \texttt{PSCOPE} : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, 1 : e,\, \sigma ,\, s,\, pc + l_b \rangle \\
              &\triangleright \langle \texttt{CSCOPE} : \mathcal{C}(S) : \texttt{PSCOPE} : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, \\
              &\quad 1 : e,\, \sigma ,\, s,\, pc + l_b + 1 \rangle
              && \text{(operational semantics)} \\
              ...
              && \text{(operational semantics)} \\
              &\triangleright \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \\
              &\quad \texttt{PSCOPE} : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, e',\, \underline{\sigma}',\, s',\, pc_b \rangle
              && \text{(operational semantics)} \\
              &= \langle \mathcal{CB}(b) : \texttt{JZ}\; pc_{\texttt{NOOP}} : \texttt{CSCOPE} : \mathcal{C}(S) : \\
              &\quad \texttt{PSCOPE} : \texttt{GOTO}\; pc_b : \texttt{NOOP},\, e',\, \underline{\sigma}',\, s',\, pc \rangle \\
  ...
  Assume that $s'(x_i) = t_i$ and $\sigma'(x_i) = \mathcal{A}(a_i, \sigma, s)$ for $i = 1,2,\dots,n$
    ...
    \[
      ...
      &\rightarrow \langle x_1 : t_1 \texttt{;} x_1 \,\texttt{:=}\, v_1\, \texttt{;} \dots \\
      &\quad \texttt{;} x_n : t_n \texttt{;} x_n \,\texttt{:=}\, v_n\, \texttt{;} S_f,\, \tilde{\sigma}', \tilde{s}' \rangle
      && [\text{call}_{\text{os}}]   \\
      &\rightarrow^\ast \langle S_f,\, \sigma' ,\, s' \rangle
      && (\text{operational semantics})   \\
      &\rightarrow^\ast \langle \texttt{return}\; a,\, \sigma'' \footnotemark ,\, s'' \rangle
      && (\text{operational semantics},\, \text{return rule})   \\
      &\rightarrow \langle \sigma[\texttt{retvar} \mapsto \mathcal{A}(a, \sigma'', s'')],\; \\
      &\quad s[\texttt{retvar} \mapsto s''(a)] \rangle
      && [\text{ret}_{\text{os}}] \\
      \end{aligned}
  \]
...
\[
  ...
  &\triangleright^\ast \langle  \texttt{CALL}\; f,\, \mathcal{A}(a_1, \sigma, s) : \dots : \mathcal{A}(a_n, \sigma, s) : e,\, \\
  &\quad \sigma,\, s,\, pc+n,\, ras \rangle
  && (\text{correctness of } \mathcal{CA})   \\
  &\triangleright \langle  P[pc_f],\, \mathcal{A}(a_1, \sigma, s) : \dots : \mathcal{A}(a_n, \sigma, s) : e,\, \\
  &\quad \sigma,\, s,\, pc_f,\, (pc+n+1) : ras \rangle
  && (\text{operational semantics}) \\
  &= \langle  \texttt{FUNCENV}\; f, t_r : \texttt{DECL}\;t_1\,x_1 : \texttt{STORE}\; x_1 :  \dots \\
  &\quad : \texttt{DECL}\;t_1\,x_1 : \texttt{STORE}\; x_n : \mathcal{C}(S_f),\, \\
  &\quad \mathcal{A}(a_1, \sigma, s) : \dots : \mathcal{A}(a_n, \sigma, s) : e,\, \\
  &\quad \sigma,\, s,\, pc_f,\, (pc+n+1) : ras \rangle
  && (\text{definition of } \mathcal{C})   \\
  &\triangleright \langle  \texttt{DECL}\;t_1\,x_1 : \texttt{STORE}\; x_1 :  \dots \\
  &\quad : \texttt{DECL}\;t_1\,x_1 : \texttt{STORE}\; x_n : \mathcal{C}(S_f),\, \\
  &\quad \mathcal{A}(a_1, \sigma, s) : \dots : \mathcal{A}(a_n, \sigma, s) : e,\, \\
  &\quad \tilde{\sigma}', \tilde{s}',\, pc_f + 1,\, (pc+n+1) : ras \rangle
  && (\text{operational semantics})   \\
  &\triangleright^\ast \langle \mathcal{C}(S_f),\, e,\, \sigma',\, s',\, pc_f + 2n+1,\, (pc+n+1) : ras \rangle
  && (\text{operational semantics})   \\
  &\triangleright^\ast \langle \mathcal{C}(\texttt{return}\; a),\, e,\, \sigma'',\, s'',\, \\
  &\quad pc_f + l + 2n+1,\, (pc+n+1) : ras \rangle
  && (\text{induction hypothesis},\, \text{return rule})   \\
  &=\langle \mathcal{CA}(a) : \texttt{RET},\, e,\, \sigma'',\, s'',\, \\
  &\quad pc_f + l + 2n+1,\, (pc+n+1) : ras \rangle
  && (\text{definition of } \mathcal{C})   \\
  &\triangleright^\ast \langle \texttt{RET},\, \mathcal{A}(a, \sigma'', s'') : e,\, \\
  &\quad \sigma'',\, s'',\, pc_f + l' + 2n+1,\, (pc+n+1) : ras \rangle
  && (\text{correctness of } \mathcal{CA})   \\
  &\triangleright \langle P[pc+n+1:],\, e,\, \sigma[\texttt{retvar} \mapsto \mathcal{A}(a, \sigma'', s'')],\; \\
  &\quad s[\texttt{retvar} \mapsto s''(a)],\, pc+n+1,\, ras \rangle
  && (\text{operational semantics}) \\
  \end{aligned}
\]
...
\footnotetext{Note that we do not cover the case where the execution of $S_f$ leads to an erroneous state, as the last semantic rule in the section \textbf{2.2.4} of this document and [$\text{err}_{\text{os}}$] rule of high-level semantics ensure the correctness of the translation in this case.}
  ...
  \item \textbf{case 11:} $P = S_1\texttt{;}S_2$ for some statements $S_1$, $S_2$\\
...
\bibitem{nielson2007} H. R. Nielson and F. Nielson, \emph{Semantics with Applications: An Appetizer}, 2007.
\end{thebibliography}
